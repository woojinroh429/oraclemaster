...
def _spill_realize(prob_info, ext, deadline):
...
def _smallright_construct(prob_info, deadline_s, small_thresh=0.60, step=1, mode="flatbl", order="rank", ext_entry=None, tiebreak="rank", ext_bay=None, feat_w=None):
    ...
    def place_custom(b,cur,is_small,windows=None,topm=None):
        ...
        _tc = [] if topm else None    # top-M collector for beam search
        ...
        return best
    # ============================================================================
    # BEAM CONSTRUCTION (deterministic deep search over the ENGINE's real placement)
    # Reuses place_custom(topm=M) so placement quality == greedy; only the greedy
    # single-commit is replaced by keeping K partial states and branching on the M
    # best-scored positions per block, pruned by objective-so-far (w1*tardiness +
    # w3*pref).  This is the fix for "shallow directional search -> local optimum":
    # a WIDE deterministic lookahead that escapes the greedy basin.  env BEAMK=<K>.
    # ============================================================================
    _BEAMK = int(os.environ.get("BEAMK", "0"))
    if _BEAMK > 0:
        _BEAMM = int(os.environ.get("BEAMM", "4"))
        _bdl = t0 + deadline_s if False else _t.time() + max(2.0, deadline_s)
        _w1 = prob_info["weights"]["w1"]; _w3 = prob_info["weights"]["w3"]
        _prefs = [B[b]["bay_preferences"] for b in range(n)]
        _order2 = sorted(range(n), key=key)
        def _beam_run():
            states = [(0.0, {}, {j: [] for j in range(m)})]   # (cost, recs, present_by_bay)
            for b in _order2:
                greedy = _t.time() > _bdl
                children = []
                for cost, srecs, spbb in states:
                    E.clear_all()
                    for _bb, _r in srecs.items():
                        try: E.add(_r["bay_id"], _bb, _r["orient_idx"], float(_r["x"]),
                                   float(_r["y"]), _r["entry_time"], _r["exit_time"])
                        except Exception: pass
                    base = rel[b]
                    exs = sorted({_r["exit_time"] for _r in srecs.values() if _r["exit_time"] >= base})
                    curseq = [base] + [t for t in exs if t > base]
                    placed = False
                    for cur_b in curseq:
                        for j in range(m):
                            present_by_bay[j] = [(bb_, ex_) for (bb_, ex_) in spbb[j] if ex_ > cur_b]
                        cands = place_custom(b, cur_b, ra[b] >= small_thresh, topm=_BEAMM)
                        if cands:
                            ex_b = cur_b + pt[b]
                            for (j, oi, ix, iy) in cands:
                                x0, y0, x1, y1 = bbox(b, oi)
                                nrec = dict(srecs)
                                nrec[b] = {"block_id": b, "bay_id": j, "x": ix, "y": iy,
                                           "orient_idx": oi, "entry_time": cur_b, "exit_time": ex_b}
                                npbb = {jj: [(bb_, ex_) for (bb_, ex_) in spbb[jj] if ex_ > cur_b]
                                        for jj in range(m)}
                                npbb[j].append(((ix + x0, iy + y0, ix + x1, iy + y1), ex_b))
                                nc = cost + _w1 * max(0, ex_b - due[b]) + _w3 * (_mxp[b] - _prefs[b][j])
                                children.append((nc, nrec, npbb))
                            placed = True
                            break
                    if not placed:
                        # forward scan as last resort (guarantee placement)
                        cur_b = (curseq[-1] + 1) if curseq else base
                        for _ in range(500):
                            for j in range(m):
                                present_by_bay[j] = [(bb_, ex_) for (bb_, ex_) in spbb[j] if ex_ > cur_b]
                            cands = place_custom(b, cur_b, ra[b] >= small_thresh, topm=1)
                            if cands:
                                j, oi, ix, iy = cands[0]; ex_b = cur_b + pt[b]
                                x0, y0, x1, y1 = bbox(b, oi)
                                nrec = dict(srecs)
                                nrec[b] = {"block_id": b, "bay_id": j, "x": ix, "y": iy,
                                           "orient_idx": oi, "entry_time": cur_b, "exit_time": ex_b}
                                ...
                                npbb[j].append(((ix + x0, iy + y0, ix + x1, iy + y1), ex_b))
                                nc = cost + _w1 * max(0, ex_b - due[b]) + _w3 * (_mxp[b] - _prefs[b][j])
                                children.append((nc, nrec, npbb)); placed = True; break
                            cur_b += 1
                    if greedy:
                        break
                if not children:
                ...
                states = children[:_BEAMK]
            br = states[0][1]
            return br if len(br) == n else {}
        return _beam_run()
