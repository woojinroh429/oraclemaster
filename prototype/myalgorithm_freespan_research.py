...
def _entry_ok(bay, present, blk):
    return len(check_entry(bay, present, blk, fast=True)) == 0


def _exit_ok(bay, present, blk):
    return len(check_exit(bay, present, blk, fast=True)) == 0
...
def _redistribute_pref(prob_info, assign, bay_unit, deadline):
    blocks = prob_info["blocks"]
    n = len(blocks)
    n_bays = len(prob_info["bays"])
    ...
    def place_in_bay(st, b, target_bay):
        bd = blocks[b]
        rt = bd["release_time"]
        pt = bd["processing_time"]
        due = bd["due_date"]
        bayobj = st.bays[target_bay]
        et_list = list(range(rt, due - pt + 1)) + list(range(due - pt + 1, rt + 20))
        for et in et_list:
            if et < rt:
                continue
            exit_t = et + pt
            present = st.present_at(target_bay, et, False)
            for oi in range(len(bd["shape"])):
                bb = _orient_bbox(bd, oi)
                for (x, y) in _candidate_positions(bayobj.width, bayobj.height,
                                                   present, bb):
                    ...
                    if not bayobj.contains_block(blk):
                        continue
                    if not _placement_feasible(st, target_bay, blk, et, exit_t):
                        continue
                    return {"block_id": b, "bay_id": target_bay, "x": x, "y": y,
                            "orient_idx": oi, "entry_time": et, "exit_time": exit_t}
        return None
    ...
    for _pass in range(3):
        if time.time() > deadline:
            break
        viol = [i for i in range(n) if assign[i]["bay_id"] != pref_bay(i)]
        cand_pairs = []
        for ii in range(len(viol)):
            for jj in range(ii + 1, len(viol)):
                i, j = viol[ii], viol[jj]
                bi, bj = assign[i]["bay_id"], assign[j]["bay_id"]
                if bi == bj:
                    continue
                pip = blocks[i]["bay_preferences"]
                pjp = blocks[j]["bay_preferences"]
                cur = (max(pip) - pip[bi]) + (max(pjp) - pjp[bj])
                swp = (max(pip) - pip[bj]) + (max(pjp) - pjp[bi])
                if swp < cur:
                    cand_pairs.append((cur - swp, i, j, bi, bj))
        if not cand_pairs:
            break
        cand_pairs.sort(reverse=True)
        cand_pairs = cand_pairs[:PAIR_CAP]
        changed = False
        for _gain, i, j, bi, bj in cand_pairs:
            if time.time() > deadline:
                break
            if assign[i]["bay_id"] != bi or assign[j]["bay_id"] != bj:
                continue
            st = _State(prob_info)
            for k, a in assign.items():
                if k == i or k == j:
                    continue
                st.add(dict(a), Block(block_id=k, block_data=blocks[k],
                                      x=a["x"], y=a["y"], orient_idx=a["orient_idx"]))
            pi_new = place_in_bay(st, i, bj)
            if pi_new is None:
                continue
            st.add(pi_new, Block(block_id=i, block_data=blocks[i],
                                 x=pi_new["x"], y=pi_new["y"],
                                 orient_idx=pi_new["orient_idx"]))
            pj_new = place_in_bay(st, j, bi)
            if pj_new is None:
                continue
            trial = dict(assign)
            trial[i] = pi_new
...
def _crane_open(prob_info, assign, bay_unit, deadline):
    blocks = prob_info["blocks"]
    n = len(blocks)
    n_bays = len(prob_info["bays"])
    ...
    def try_place(st, b, j):
        bd = blocks[b]
        rt = bd["release_time"]
        pt = bd["processing_time"]
        due = bd["due_date"]
        for et in range(rt, max(rt, due - pt) + 1):
            bayobj = st.bays[j]
            present = st.present_at(j, et, False)
            for oi in range(len(bd["shape"])):
                bb = _orient_bbox(bd, oi)
                for (x, y) in _candidate_positions(bayobj.width, bayobj.height,
                                                   present, bb):
                    blk = Block(block_id=b, block_data=bd, x=x, y=y, orient_idx=oi)
                    if not bayobj.contains_block(blk):
                        continue
                    if not _placement_feasible(st, j, blk, et, et + pt):
                        continue
                    return {"block_id": b, "bay_id": j, "x": x, "y": y,
                            "orient_idx": oi, "entry_time": et, "exit_time": et + pt}
        return None

    def find_obstructors(b, j):
        bd = blocks[b]
        rt = bd["release_time"]
        pt = bd["processing_time"]
        due = bd["due_date"]
        ...
        for et in range(rt, max(rt, due - pt) + 1):
            ...
            for oi in range(len(bd["shape"])):
                ...
                for (x, y) in _candidate_positions(bayobj.width, bayobj.height,
                    ...
                    if not bayobj.contains_block(blk):
                        continue
                    obs = check_entry(bayobj, present, blk, fast=False)
                    for o in obs:
                        if o.existing_block.block_id != b:
                            obstructors.add(o.existing_block.block_id)
        return obstructors

    cur_obj = _objective(list(assign.values()), prob_info, bay_unit)[0]
    viol = sorted(
        [b for b in range(n) if assign[b]["bay_id"] != pref_bay(b)],
        key=lambda b: -(max(blocks[b]["bay_preferences"])
                        - blocks[b]["bay_preferences"][assign[b]["bay_id"]]))
    for b in viol:
        if time.time() > deadline - 0.3:
            break
        pb = pref_bay(b)
        obs = find_obstructors(b, pb)
        if not obs:
            continue
        for a_id in list(obs)[:3]:
            if time.time() > deadline - 0.3:
                break
            trial = dict(assign)
            st = build_state({a_id, b})
            a_bd = blocks[a_id]
            a_bays = sorted(range(n_bays), key=lambda j: -a_bd["bay_preferences"][j])
            a_new = None
            for jj in a_bays[:2]:
                a_new = try_place(st, a_id, jj)
                if a_new:
                    break
            if a_new is None:
                continue
            st.add(a_new, Block(block_id=a_id, block_data=a_bd,
                                x=a_new["x"], y=a_new["y"], orient_idx=a_new["orient_idx"]))
            b_new = try_place(st, b, pb)
            if b_new is None:
                continue
            trial[a_id] = a_new
            trial[b] = b_new
...
def _corridor_open(prob_info, assign, bay_unit, deadline):
    blocks = prob_info["blocks"]
    n = len(blocks)
    n_bays = len(prob_info["bays"])
    ...
    for i in range(n):
        demand[pref_bay(i)] += 1
    pop = max(range(n_bays), key=lambda j: demand[j])

    viol_want = [b for b in range(n)
                 if assign[b]["bay_id"] != pop and pref_bay(b) == pop]
    if len(viol_want) < 2:
        return assign
    residents = [b for b in range(n) if assign[b]["bay_id"] == pop]
    if not residents:
        return assign
    ...
    def removal_cost(rb):
        p = blocks[rb]["bay_preferences"]
        srt = sorted(range(n_bays), key=lambda j: -p[j])
        return max(p) - max(p[j] for j in range(n_bays) if j != pop)
    residents_sorted = sorted(residents, key=removal_cost)

    tried = 0
    MAX_REMOVE_TRIALS = 12
    for rem_b in residents_sorted:
        if tried >= MAX_REMOVE_TRIALS or time.time() > deadline - 0.3:
            break
        tried += 1
        st = build_state({rem_b})
        rem_bays = [j for j in sorted(range(n_bays),
                                      key=lambda j: -blocks[rem_b]["bay_preferences"][j])
                    if j != pop]
        rem_new = None
        for jj in rem_bays:
            rem_new = try_place_in(st, rem_b, jj)
            if rem_new:
                break
        if rem_new is None:
            continue
        ...
        trial = dict(assign)
        trial[rem_b] = rem_new
        work = build_state({rem_b})
        work.add(rem_new, Block(block_id=rem_b, block_data=blocks[rem_b],
                                x=rem_new["x"], y=rem_new["y"],
                                orient_idx=rem_new["orient_idx"]))
        admitted = 0
        for vb in sorted(viol_want,
                         key=lambda b: -(max(blocks[b]["bay_preferences"])
                                         - blocks[b]["bay_preferences"][assign[b]["bay_id"]])):
            if time.time() > deadline - 0.3:
                break
            work_v = _State(prob_info)
            for i, a in trial.items():
                if i == vb:
                    continue
                work_v.add(dict(a), Block(block_id=i, block_data=blocks[i],
                                          x=a["x"], y=a["y"], orient_idx=a["orient_idx"]))
            v_new = try_place_in(work_v, vb, pop)
            if v_new:
                trial[vb] = v_new
        admitted = sum(1 for vb in viol_want if trial[vb]["bay_id"] == pop)
        if admitted < 1:
            continue
        sol = _build_operations(list(trial.values()))
        chk = check_feasibility(prob_info, sol)
...
def _atc_order(prob_info, k_param):
    bd = prob_info["blocks"]
    n = len(bd)
    p_avg = sum(b["processing_time"] for b in bd) / max(1, n)
    p_avg = max(p_avg, 1e-6)

    def prio(bid):
        b = bd[bid]
        p = max(b["processing_time"], 1e-6)
        slack = max(0.0, b["due_date"] - p - b["release_time"])
        return (1.0 / p) * math.exp(-slack / (k_param * p_avg))

    return sorted(range(n), key=lambda bid: (-prio(bid),
                                             bd[bid]["due_date"],
                                             bd[bid]["release_time"]))
...
def _worker_entry(args):
    ...
    try:
        ...
        pass
    prob_info, timelimit, seed, shared, lock, worker_id, cwd, use_cpp, il_mode, nfp_mode, absorb, cpp_engine, eng_area_commit, hybrid_flag, coreperi_flag, repair_ok = args
    try:
            ...
            def _try_smallright():
                ...
                def _attempt(_step, _mode="flatbl"):
                    ...
                    try:
                        ...
                        return None
                    return None
                _best = _attempt(2, _primary_mode)   # fast safety net (bottom-left OR left-bottom)
                if _hyb_deadline - time.time() > 6.0:
                    _hi = _attempt(1, _primary_mode) # higher-quality; keep iff it finished + better
                    if _hi is not None and (_best is None or _hi[1] < _best[1]):
                        _best = _hi
                # Diagonal Fill variant (Kwon & Lee 2015): a tighter corner-packing
                # rule that beats bottom-left on some P6 instances (prob_37/40 ~9%)
                # and loses on others; best-of keeps whichever wins per instance, so
                # it can only help.  Only attempted if wall time remains after the
                # primary flat_bl attempts, so it never starves them.
                if _hyb_deadline - time.time() > 6.0:
                    _dg = _attempt(2, "diagonal")
                    if _dg is not None and (_best is None or _dg[1] < _best[1]):
                        _best = _dg
                    if _hyb_deadline - time.time() > 6.0:
                        _dg1 = _attempt(1, "diagonal")
                        if _dg1 is not None and (_best is None or _dg1[1] < _best[1]):
                            _best = _dg1
                # LEFT-BOTTOM tail variant (was a primary before bigleft dominated
                # it): forces ALL blocks left, still wins on some instances, so keep
                # it in best-of.  Time-gated -> never starves primaries.
                # LEFT-BOTTOM variant.  v33 REGRESSION FIX: v33 demoted leftbottom
                # from a primary (full budget + step=1) to a step=2-only tail, which
                # cost P4 +13.7% (3.95M -> 4.49M) because leftbottom is the P4 winner
                # (mid-density: forcing ALL blocks left leaves a big right free span
                # that suits P4's block mix).  v34 restores step=1 leftbottom so its
                # higher-quality packing is available in best-of again.  bigleft still
                # wins P6, leftbottom wins P4 -> best-of keeps each -> no regression.
                if _hyb_deadline - time.time() > 6.0:
                    _lb2 = _attempt(2, "leftbottom")
                    if _lb2 is not None and (_best is None or _lb2[1] < _best[1]):
                        _best = _lb2
                    if _hyb_deadline - time.time() > 6.0:
                        _lb1 = _attempt(1, "leftbottom")
                        if _lb1 is not None and (_best is None or _lb1[1] < _best[1]):
                            _best = _lb1
                # BIG-LEFT variant: large blocks fill left-first (cluster to one side,
                # leaving a big contiguous free span for later big blocks), small
                # blocks keep gap-filling.  Measured to beat both flatbl and
                # leftbottom on dense instances (prob_27 -7.9%, 37 -4.7%, 38 -3.0%,
                # 39 -2.0%, 40 -4.4% Z1).  Runs as primary on odd workers; here it is
                # the tail variant for even (flatbl-primary) workers so best-of always
                # sees it.  Time-gated -> never starves; feasibility-gated -> never -1.
                if _hyb_deadline - time.time() > 6.0:
                    _bl2 = _attempt(2, "bigleft")
                    if _bl2 is not None and (_best is None or _bl2[1] < _best[1]):
                        _best = _bl2
                    if _hyb_deadline - time.time() > 6.0:
                        _bl1 = _attempt(1, "bigleft")
                        if _bl1 is not None and (_best is None or _bl1[1] < _best[1]):
                            _best = _bl1
                return _best
...
def _smallright_construct(prob_info, deadline_s, small_thresh=0.60, step=1, mode="flatbl", order="rank", ext_entry=None, tiebreak="rank", ext_bay=None, feat_w=None):
    ...
    def _free_span_with(occ_base, bw, wx, w, wy, band_top):
        ...
        if bw>c2:
            ...
            if _g>fm: fm=_g
        return fm
    def place_custom(b,cur,is_small):
        ...
        for j in _bl_b:
            ...
            for oi in range(len(B[b]["shape"])):
                ...
                for ix in range(lo_x, hi_x+1, step):
                    for iy in range(lo_y, hi_y+1, step):
                        if E.placement_feasible(j,b,oi,float(ix),float(iy),cur,ex):
                            ...
                            if mode=="interlock":
                                ...
                                # FREE-SPAN DIRECT: place EVERY block (big or small) so
                                # the largest remaining contiguous floor gap is maximised.
                                # This is the principled generalisation of the directional
                                # heuristics (bigleft/right/corner) -- instead of a fixed
                                # wall, it clusters adaptively wherever it keeps the free
                                # span largest (e.g. abut an existing block to extend the
                                # gap).  Big blocks tiebreak by flatness then bottom-left.
                                # MULTI-BAND free-span: max free x-gap in each of K
                                # height bands; the MIN across bands ~= width of the
                                # widest full-height free column left after placement
                                # (a 2D proxy the 1D bottom-band metric lacks).
                                _K=4; _mn=1e18
                                for _kk in range(_K):
                                    _ya=bh_j*_kk/_K; _yb=bh_j*(_kk+1)/_K
                                    _iv=[]
                                    for (_bx0,_by0,_bx1,_by1),_ex in present_by_bay[j]:
                                        if _ex<=cur: continue
                                        if _by1>_ya and _by0<_yb: _iv.append((_bx0,_bx1))
                                    if wy+h>_ya and wy<_yb: _iv.append((wx,wx+w))
                                    _iv.sort(); _fm=0.0; _c2=0.0
                                    for _a,_b in _iv:
                                        if _a>_c2:
                                            _g=_a-_c2
                                            if _g>_fm: _fm=_g
    ...
    while True:
        ...
        for b in _pend_sorted:
            ...
            if placed: _hq.heappush(eh,(recs[b]["exit_time"],b)); pl.append(b)
    ...
    try:
        with multiprocessing.Manager() as manager:
            ...
            args = [(prob_info, _remaining, _WORKER_SEEDS[i % len(_WORKER_SEEDS)],
                     ...
                     _hybrid_for(i), _coreperi_for(i), _p5_band)
