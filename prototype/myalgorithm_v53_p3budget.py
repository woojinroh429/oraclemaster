...
def _solve_once_impl(prob_info, timelimit=60, seed=12345,
    ...
    if _low_density:
        ...
        _tr_def = "0.65" if (worker_id or 0) % 2 == 0 else "0.0"
    ...
    if _low_density and n_blocks > 0 and verified_sol is not None:
        ...
        if time.time() < deadline - 0.6:
            try:
                _sa_dl = time.time() + _sa1_frac * (deadline - time.time())
        ...
        if os.environ.get("EXACT", "1") == "1" and time.time() < deadline - 6.0:
            try:
                ...
                _res = _exact_reassign(prob_info, bay_unit,
                                       min(deadline - 2.0, time.time() + _ex_cap),
                                       mode=_exmode)
                if _res is not None:
                    ...
                    if (_seed_ex is not None and _seed_z1 <= max(3.0, 0.03 * n_blocks)
                            and time.time() < deadline - 0.6):
                        _rp_dl = time.time() + 0.6 * (deadline - time.time())
