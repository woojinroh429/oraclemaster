...
def _sa_reassign(prob_info, assign, bay_unit, deadline, rng):
...
def _exact_reassign(prob_info, bay_unit, deadline, mip_cap=6.0):
    ...
    try:
        ...
        for j in range(m):
            for t in sorted(set(rel)):
                ...
                if present:
                    mdl.Add(sum(x[b][j] * area[b] for b in present) <= int(cap[j]))
        Z3 = sum(x[b][j] * (mxp[b] - B[b]["bay_preferences"][j])
                 for b in range(n) for j in range(m))
        mdl.Minimize(W2 * Mv + W3 * SC * Z3)
        slv = cp_model.CpSolver()
        slv.parameters.max_time_in_seconds = max(1.0, min(mip_cap, deadline - _t.time() - 4.0))
        ...
        for mode in ("prefaware", "bigleft"):
            if deadline - _t.time() < 1.0:
                break
            recs = _smallright_construct(prob_info, max(1.0, deadline - _t.time()),
                                         step=1, mode=mode, ext_bay=ext)
            if recs and len(recs) == n:
                sol = _build_operations([recs[b] for b in range(n)])
                ck = check_feasibility(prob_info, sol)
                if ck.get("feasible") and float(ck["objective"]) < best_obj:
                    best_obj = float(ck["objective"])
                    best = {b: dict(recs[b]) for b in range(n)}
        return best
    except Exception:
        return None
    ...
    if _low_density and n_blocks > 0 and verified_sol is not None and time.time() < deadline - 6.0:
        try:
            ex = _exact_reassign(prob_info, bay_unit,
                                 min(deadline - 2.0, time.time() + 15.0))
