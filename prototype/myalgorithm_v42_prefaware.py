    ...
    if hybrid_flag or coreperi_flag:
        ...
        try:
            ...
            def _try_smallright():
                ...
                if (os.environ.get("PREFAWARE", "1") == "1"
                        and len(prob_info["blocks"]) < 200):
                    # PREFERENCE-AWARE Z3 lever: mid/low-density objective is ~50% Z3
                    # (bay-preference penalty) which the packing modes ignore; prefaware
                    # places blocks in their preferred bay first.  best-of keeps min ->
                    # Z1-dominated instances keep bigleft (never-worse).  Measured
                    # prob_24 -42%, prob_28 -7.7%.  Gated to n<200: on large the budget
                    # is spent by the dedicated bigleft-completion worker and those are
                    # Z1-dominated, so prefaware there only steals wall time.
                    _tails.append("prefaware")
                for _tm in _tails:
                    _tail(_tm)
                return _best[0]
