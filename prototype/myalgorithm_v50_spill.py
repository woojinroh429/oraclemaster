...
def _exact_reassign(prob_info, bay_unit, deadline, mip_cap=6.0):
    ...
    try:
        ...
        for _bit in range(5):
            ...
            if _bit == 0 and os.environ.get("SPILL", "1") == "1":
                try:
                    _spb = _spill_realize(prob_info, ext,
                                          min(deadline - 1.0, _t.time() + max(4.0, 0.5 * (deadline - _t.time()))))
                    if _spb is not None and _spb[0] < best_obj:
