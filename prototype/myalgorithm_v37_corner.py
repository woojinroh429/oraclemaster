    ...
    if hybrid_flag or coreperi_flag:
        ...
        try:
            ...
            def _try_smallright():
                ...
                # disables (=v36).
                # Gate to n<=160: on large instances the 4x step=1 corner search cannot
                # complete, starves the worker, and replacing its bigleft primary
                # regresses high-density (prob_38 n=250 -2.5%).  n<=160 keeps the mid-
                # density gains (prob_21/24/25/28 +3.6..5.5%) with no large-n regression.
                _corner_primary = (os.environ.get("DIRS_EXTRA", "1") == "1"
                                   and (_wid % 2 == 1)
                                   and len(prob_info["blocks"]) <= 160)
                ...
                def _attempt(_step, _mode="flatbl"):
                    ...
                    return None
                if _corner_primary:
                    # corner-best-of as PRIMARY: run step=1 (quality) on EACH corner
                    # while time permits (step=2 fallback when low on time), keep best.
                    # A step=2 probe misranks corners (step2 winner != step1 winner), so
                    # we must evaluate each at quality to find the true winner.
                    _best = None
                    for _cm in ("cornerTL", "cornerBL", "cornerTR", "cornerBR"):
                        _cc = _attempt(1 if _hyb_deadline - time.time() > 5.0 else 2, _cm)
