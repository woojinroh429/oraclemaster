        ...
        try:
            ...
            def _try_smallright():
                ...
                if _corner_primary:
                    ...
                    # baseline (never starve).  (2) bigleft step=1 = the v36-quality
                    # primary AND a live MEASUREMENT of one step=1 construction's cost.
                    # (3) run each corner at step=1 only while a full one still fits in
                    # the remaining budget (using the measured cost) -> on fast instances
                    # all 4 corners run (mid-density gains); on slow/large ones few or
                    # none run and bigleft stands (no starvation, no regression).  A
                    # step=2 probe misranks corners, so each corner is evaluated at
                    # step=1 quality.  best-of => never worse than bigleft.
                    _t_s2 = time.time()
                    _best = _attempt(2, "bigleft")        # fast baseline + measure step=2 cost
                    _s1_est = (time.time() - _t_s2) * 3.0 # step=1 ~ 3x step=2 (empirical)
                    if _hyb_deadline - time.time() > _s1_est * 1.15 + 2.0:
                        # step=1 constructions fit -> run corners at quality (don't waste
                        # budget on a redundant bigleft step=1 first; other workers cover
                        # bigleft, and the step=2 baseline here already guards).
                        for _cm in ("cornerTL", "cornerBL", "cornerTR", "cornerBR"):
                            if _hyb_deadline - time.time() > _s1_est * 1.15 + 2.0:
                                _cc = _attempt(1, _cm)
                                if _cc is not None and (_best is None or _cc[1] < _best[1]):
                                    _best = _cc
                            else:
                                break
                    else:
