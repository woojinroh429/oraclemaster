...
def _worker_entry(args):
    ...
    try:
        ...
        pass
    prob_info, timelimit, seed, shared, lock, worker_id, cwd, use_cpp, il_mode, nfp_mode, absorb, cpp_engine, eng_area_commit, hybrid_flag, coreperi_flag = args
    try:
...
def _smallright_construct(prob_info, deadline_s, small_thresh=0.60, step=1, mode="flatbl", order="rank", ext_entry=None, tiebreak="rank", ext_bay=None, feat_w=None):
    ...
    if feat_w is not None:
        ...
        _F={
            ...
            "work":_mkr(_work,True), "pref":_mkr(_pref,True),
        }
    ...
    try:
        with multiprocessing.Manager() as manager:
            ...
            def _coreperi_for(i):
                ...
                # hybrid worker when the engine is available (>=4 workers so we don't
                # sacrifice a primary lane).  Additive best-of => captures coreperi's
                # wins (prob_33 -14.4%) with zero regression elsewhere.  Falls back to
                # the numba/simple path inside the worker if coreperi fails (-1 safety).
                return (i == n_workers - 1) and n_workers >= 4 and HAVE_OGC_FAST and HAVE_CPP
