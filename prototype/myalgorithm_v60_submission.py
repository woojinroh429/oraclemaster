...
def _solve_once_impl(prob_info, timelimit=60, seed=12345,
    ...
    # --- Space-time 3DTCS construction (default on; ST3MOD workers).  Full-scan
    # placement scored to maximise 3D-total-contiguous-surface contact in (x,y,t)
    # -> tighter space-time packing than bottom-left.  Measured net-positive across
    # 40 instances (prob_34 -51.8%, prob_35 -32%, prob_7 -19%, prob_28 -11.7%;
    # small losses only on tiny-objective instances), always crane-feasible; the
    # infeasible/incomplete builds are rejected below by check_feasibility, so it is
    # strictly best-of with the engine/Python builds.  Adaptive step-down keeps it
