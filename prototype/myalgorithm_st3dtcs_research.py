...
def _load_st3():
    ...
    Returns the module or None if unavailable (grader may lack the .so) -- callers
    fall back to the shipped construction, so absence is safe."""
...
_IL_MODE = False
_ST_FULLSTEP = int(os.environ.get("STFULLSTEP", "0"))   # >0: full grid scan step for _st_place_block
...
# ----------------------------------------------------------------------------
# Space-time (FPGA-temporal-placement style) construction.  Instead of the fixed
# bottom-left tie-break, placements are scored to MAXIMISE 3D-total-contiguous-
# surface (3DTCS) contact in (x, y, t): shared spatial edges with time-overlapping
# neighbours + walls, plus temporal-face footprint overlap with blocks that abut
# in time.  Maximising contact minimises space-TIME fragmentation, so later blocks
# fit with less delay.  Objective terms (tardiness, preference, load) stay PRIMARY
# and are derived from w1/w2/w3 -- no fitted parameters -- so the method is general
# (overfitting-resistant), not tuned to the training bays.  Exact crane/overlap
# feasibility is reused verbatim.  env-gated STCONSTRUCT; competes best-of vs the
# shipped construction.
# ----------------------------------------------------------------------------
def _st_contact(state, bay_id, nbb, entry_t, exit_t, bay):
    """3DTCS space-time contact of a block with abs-bbox `nbb` occupying
    [entry_t, exit_t) in bay `bay_id`: shared spatial edge length with
    time-overlapping neighbours and walls, plus temporal-face footprint overlap
    with blocks abutting in time.  Higher = denser space-time packing."""
    nx0, ny0, nx1, ny1 = nbb
    c_spatial = 0.0
    c_temporal = 0.0
    if nx0 <= 1e-9:
        c_spatial += (ny1 - ny0)
    if nx1 >= bay.width - 1e-9:
        c_spatial += (ny1 - ny0)
    if ny0 <= 1e-9:
        c_spatial += (nx1 - nx0)
    if ny1 >= bay.height - 1e-9:
        c_spatial += (nx1 - nx0)
    for (en, ex, _blk, ob) in state.timeline[bay_id]:
        ox0, oy0, ox1, oy1 = ob
        if en < exit_t and ex > entry_t:            # spatial edge needs time overlap
            yov = min(ny1, oy1) - max(ny0, oy0)
            if yov > 1e-9 and (abs(nx1 - ox0) < 1e-9 or abs(ox1 - nx0) < 1e-9):
                c_spatial += yov
            xov = min(nx1, ox1) - max(nx0, ox0)
            if xov > 1e-9 and (abs(ny1 - oy0) < 1e-9 or abs(oy1 - ny0) < 1e-9):
                c_spatial += xov
        if abs(ex - entry_t) < 1e-9 or abs(en - exit_t) < 1e-9:   # temporal face
            aov = (min(nx1, ox1) - max(nx0, ox0)) * (min(ny1, oy1) - max(ny0, oy0))
            if aov > 1e-9:
                c_temporal += aov
    # scale area-unit temporal contact to length-unit spatial contact by the bay's
    # short dimension -> comparable magnitudes with NO fitted weight.
    scale = 1.0 / max(1.0, float(min(bay.width, bay.height)))
    return c_spatial + c_temporal * scale


def _st_place_block(state, bid, bay_order, deadline):
    """Like _try_place_block but the tie-break MAXIMISES 3DTCS space-time contact
    instead of bottom-left (y, x).  Primary key stays (tardiness, preference,
    load) so the objective is respected exactly; contact only breaks ties among
    equally-good-on-objective placements.  Returns the placement dict or None."""
    prob = state.prob
    bd = prob["blocks"][bid]
    _cpp_active = getattr(state, "_cpp", None) is not None
    _pf = _cpp_placement_feasible if _cpp_active else _placement_feasible
    rt = bd["release_time"]; pt = bd["processing_time"]; due = bd["due_date"]
    prefs = bd["bay_preferences"]; n_orients = len(bd["shape"])
    mp = max(prefs)
    ...
    best = None
    best_key = None            # (tard, pref_pen, load_metric, -contact, y, x, entry_t)
    for entry_t in cand_times:
        if time.time() > deadline:
            break
        if best_key is not None and entry_t + pt - due > best_key[0] + 1e-9:
        ...
        tard = max(0.0, exit_t - due)
        for bay_id in pref_bay_order:
            pref_pen = mp - prefs[bay_id]
            if best_key is not None and (tard, pref_pen) > (best_key[0], best_key[1]):
                break
            bay = state.bays[bay_id]
            present_entry = state.present_at(bay_id, entry_t, False)
            load_metric = bay_u[bay_id] * (cur_load[bay_id] + wl)
            for oi in range(n_orients):
                bb = _orient_bbox(bd, oi)
                lx0, ly0, lx1, ly1 = bb
                if _ST_FULLSTEP > 0:
                    # full grid scan (complete candidate set) -- the fair test of
                    # 3DTCS scoring vs the engine's bottom-left, same completeness.
                    import math as _m
                    xs = range(int(_m.ceil(-lx0)), int(_m.floor(bay.width - lx1)) + 1, _ST_FULLSTEP)
                    ys = range(int(_m.ceil(-ly0)), int(_m.floor(bay.height - ly1)) + 1, _ST_FULLSTEP)
                    cands = [(x, y) for y in ys for x in xs]
                else:
                    cands = _candidate_positions(bay.width, bay.height, present_entry, bb)
                seen_xy = set()
                for (x, y) in cands:
                    if (x, y) in seen_xy:
                        continue
                    ...
                    if not (lx0 + x >= -1e-9 and ly0 + y >= -1e-9
                            ...
                            and ly1 + y <= bay.height + 1e-9):
                        continue
                    # cheap primary-key prune before the (costlier) feasibility +
                    # contact: skip if it can't beat best on (tard, pref, load).
                    if best_key is not None and (tard, pref_pen, load_metric) > best_key[:3]:
                        continue
                    blk = (_LiteBlock(bid, oi, x, y, bd) if _cpp_active
                           else Block(block_id=bid, block_data=bd, x=x, y=y, orient_idx=oi))
                    if not _pf(state, bay_id, blk, entry_t, exit_t):
                        continue
                    nbb = (lx0 + x, ly0 + y, lx1 + x, ly1 + y)
                    contact = _st_contact(state, bay_id, nbb, entry_t, exit_t, bay)
                    cand_key = (tard, pref_pen, load_metric, -contact, y, x, entry_t)
                    if best_key is None or cand_key < best_key:
                        best_key = cand_key
                        best = {"block_id": bid, "bay_id": bay_id, "x": x, "y": y,
                                "orient_idx": oi, "entry_time": entry_t, "exit_time": exit_t}

    if best is None:
        return _try_place_block(state, bid, bay_order, deadline)   # guaranteed-seat fallback
    blk_best = Block(block_id=bid, block_data=bd,
                     x=best["x"], y=best["y"], orient_idx=best["orient_idx"])
    state.add(best, blk_best)
    return best
...
def _st_construct(prob_info, order, deadline, dynamic=False):
    """Space-time construction.  `order` seeds the sequence (release-primary, the
    proven-best structure).  With `dynamic`, re-selects at each step the unplaced
    block with least schedule slack (due-release-pt) so urgent blocks claim good
    space-time seats first.  Placement uses _st_place_block (contact-maximising)."""
    state = _CppState(prob_info) if (HAVE_CPP and os.environ.get("CPPPOLISH", "1") == "1") else _State(prob_info)
    n_bays = len(prob_info["bays"])
    ...
    if not dynamic:
        for bid in order:
            if time.time() > deadline:
                break
            _st_place_block(state, bid, bays, deadline)
        return state
    blocks = prob_info["blocks"]
    remaining = list(order)
    def _slack(b):
        bd = blocks[b]
        return bd["due_date"] - bd["release_time"] - bd["processing_time"]
    remaining.sort(key=lambda b: (_slack(b), blocks[b]["release_time"]))
...
def _st_construct_cpp(prob_info, order, deadline, step=1):
    """C++-backed space-time (3DTCS) construction via the st3dtcs module.  Same
    contact-maximising placement as _st_place_block but the full grid scan +
    feasibility + contact run in C++, so it is fast enough for the real budget.
    Falls back to the pure-Python path if the module is unavailable, and to
    _try_place_block per block if a scan finds no seat.  Returns a state."""
    _m = _load_st3()
    if _m is None:
        return _st_construct(prob_info, order, deadline)
    import numpy as _np
    blocks = prob_info["blocks"]
    n_bays = len(prob_info["bays"])
    ...
    cur_load = [0.0] * n_bays
    # adaptive step-down: keep the fine (high-quality) scan while on schedule, but
    # coarsen for the remaining blocks if the build falls behind, so large/dense
    # instances still COMPLETE within budget instead of dying part-placed.  Robust
    # to machine speed/contention because it reacts to wall-clock, not a size guess.
    _t0 = time.time()
...
def _regret_reinsert(state, E, ins, n_bays, deadline, k=2):
    """Regret-k ALNS repair (env REGRET, needs CPPREPAIR).  Instead of seating each
    removed block at its greedy-best position in a fixed order, repeatedly seat the
    block with the largest REGRET -- the gap between its best and k-th-best bay
    insertion cost -- so blocks that would be expensive to place later go first.

    This is the Ropke-Pisinger regret heuristic adapted to the bay dimension; it is
    the piece that makes LARGE destroys productive (plain greedy just rebuilds the
    same basin because each block still lands in its locally-best seat).  Cost is
    O(pool^2 * n_bays) find_best_placement calls, so the caller gates it OFF on
    dense instances where each call is tens of ms.  Returns True iff all seated."""
    prob = state.prob
    blocks = prob["blocks"]
    wq = prob["weights"]
    w1 = wq["w1"]; w3 = wq["w3"]
    ...
    while pool:
        ...
        pick_score = None
        pick_pl = None
        force_b = None            # block infeasible at natural times -> must fallback
        for b in pool:
            bd = blocks[b]
            rt = bd["release_time"]; due = bd["due_date"]; prefs = bd["bay_preferences"]
            mp = max(prefs)
            base = {int(rt)}
            for j in bay_list:
                for (en, ex, _bb, _bx) in state.timeline[j]:
                    if ex >= rt:
                        base.add(int(ex))
            base = sorted(base)
            costs = []
            for bay in bay_list:
                res = E.find_best_placement(b, [bay], base)
                if res[0]:
                    _, by, oi, x, y, en, ex = res
                    c = w1 * max(0.0, ex - due) + w3 * (mp - prefs[int(by)])
                    costs.append((c, res))
            if not costs:
                force_b = b
                break
            ...
            best_c = costs[0][0]
            reg = sum(costs[i][0] - best_c for i in range(1, min(k, len(costs))))
            score = (reg, -best_c)
        ...
        if force_b is not None:
            b = force_b
            rt = blocks[b]["release_time"]
            latest = int(rt)
            for j in bay_list:
                for (en2, ex2, _b2, _x2) in state.timeline[j]:
                    latest = max(latest, int(ex2))
            et = max(int(rt), latest); g = 0; res = (0,)
            while not res[0] and g < 5000:
                g += 1
                res = E.find_best_placement(b, bay_list, [et])
                if not res[0]:
                    et += 1
            if not res[0]:
                return False
            pick_b = b
            pick_pl = res
        _, by, oi, x, y, en, ex = pick_pl
        a = {"block_id": pick_b, "bay_id": int(by), "x": int(x), "y": int(y),
             "orient_idx": int(oi), "entry_time": int(en), "exit_time": int(ex)}
        blk = Block(block_id=pick_b, block_data=blocks[pick_b],
                    x=int(x), y=int(y), orient_idx=int(oi))
        state.add(a, blk)
        E.add(int(by), pick_b, int(oi), float(x), float(y), int(en), int(ex))
        pool.remove(pick_b)
    return True


def _fast_reinsert(state, E, ins, n_bays, deadline):
    """Fast greedy ALNS repair (env FASTREPAIR, needs CPPREPAIR engine).  Same
    candidate enumeration + scoring key as _try_place_block, but each candidate is
    tested with the raw C++ E.placement_feasible (~5 us) instead of allocating a
    Block and running _cpp_placement_feasible (compute_bbox + check_entry_fast +
    shapely fallback) or find_best_placement (a full internal grid scan, 12-56 ms).
    Placement decisions are identical to _try_place_block; only the wall-time per
    repair drops ~10-50x -> the throughput needed to capture the search headroom."""
    prob = state.prob
    blocks = prob["blocks"]
    bay_u = _bay_unit_weights(prob["bays"])
    bay_list = list(range(n_bays))
    ...
    for b, a in state.assign.items():
        E.add(int(a["bay_id"]), b, int(a["orient_idx"]), float(a["x"]),
              float(a["y"]), int(a["entry_time"]), int(a["exit_time"]))
        cur_load[a["bay_id"]] += blocks[b]["workload"]
    for bid in ins:
        if time.time() > deadline:
            return False
        bd = blocks[bid]
        rt = bd["release_time"]; pt = bd["processing_time"]; due = bd["due_date"]
        prefs = bd["bay_preferences"]; mp = max(prefs); wl = bd["workload"]
        n_orients = len(bd["shape"])
        base = {int(rt)}
        for j in bay_list:
            for (en, ex, _b, _bb) in state.timeline[j]:
                if ex >= rt:
                    base.add(int(ex))
        cand_times = sorted(base)
        pref_bay_order = sorted(bay_list, key=lambda j: -prefs[j])
        best = None
        best_key = None
        for entry_t in cand_times:
            if best_key is not None and entry_t + pt - due > best_key[0] + 1e-9:
                ...
                for oi in range(n_orients):
                    ...
                    for (x, y) in cands:
                        if (x, y) in seen:
                            continue
                        seen.add((x, y))
                        ck = (tard, pref_pen, load_metric, y, x, entry_t)
                        if best_key is not None and ck >= best_key:
                            continue
                        if not (lx0 + x >= -1e-9 and ly0 + y >= -1e-9
                                and lx1 + x <= bay.width + 1e-9
                                and ly1 + y <= bay.height + 1e-9):
                            continue
                        if not E.placement_feasible(bay_id, bid, oi, float(x), float(y),
        ...
        if best is None:
            ...
            if not res[0]:
                return False
            _, by, oi, x, y, en, ex = res
            best = {"block_id": bid, "bay_id": int(by), "x": int(x), "y": int(y),
                    "orient_idx": int(oi), "entry_time": int(en), "exit_time": int(ex)}
        a = dict(best)
        blk = Block(block_id=bid, block_data=bd,
                    x=a["x"], y=a["y"], orient_idx=a["orient_idx"])
        state.add(a, blk)
        E.add(int(a["bay_id"]), bid, int(a["orient_idx"]), float(a["x"]),
              float(a["y"]), int(a["entry_time"]), int(a["exit_time"]))
        cur_load[a["bay_id"]] += wl
    return True
...
def _alns(prob_info, state, bay_unit, deadline, rng, use_gls=False,
    ...
    _regret_on = os.environ.get("REGRET", "0") == "1"
    _fast_on = os.environ.get("FASTREPAIR", "0") == "1"
    _regret_k = int(os.environ.get("REGRETK", "2"))
    _repair_E = None
    if _cpprepair_on or _regret_on or _fast_on:
        try:
            _repair_E = _ogc_fast_engine(prob_info)
            _cpprepair_on = True
            # FBCAP (env): throttle find_best_placement's internal scan via
            # set_caps(256, FBCAP).  Measured: cap 4 ~= 4x faster placement at ~10%
            # per-move tardiness loss -- trades per-repair quality for ~4x ALNS
            # throughput (the search headroom shows more iterations win on dense).
            _fbcap = int(os.environ.get("FBCAP", "0"))
            if _fbcap > 0:
                try:
                    _repair_E.set_caps(256, _fbcap)
                except Exception:
                    pass
        except Exception:
            _cpprepair_on = False
    ...
    while time.time() < deadline:
        ...
        if _fast_on:
            ok = _fast_reinsert(trial, _repair_E, ins, n_bays, deadline)
        elif _regret_on:
            ok = _regret_reinsert(trial, _repair_E, ins, n_bays, deadline, _regret_k)
        elif _cpprepair_on:
    ...
    # restore the shared engine's full-scan caps so later phases (polish/relocator,
    # which reuse the cached _ogc_fast_engine) are not left throttled by FBCAP.
    if _repair_E is not None and os.environ.get("FBCAP", "0") != "0":
        try:
            _repair_E.set_caps(1 << 20, 1 << 20)
        except Exception:
            pass
...
def _beam_realize(prob_info, ext, deadline, K=6, M=3):
    """JOINT reposition+assignment: realize assignment `ext` with a K-state corridor
    beam instead of _spill_realize's bottom-left first-fit.  For each block (big-first)
    it branches over the top-M positions in its assigned bay that LEAVE the largest
    descent corridor (max empty rect) -- a lookahead the greedy first-fit lacks -- so
    MORE blocks seat on-time in-preference, raising each bay's realised capacity, which
    (via _exact_reassign's Benders feedback) lets the next CP-SAT keep more in-pref ->
    lower Z3.  Spills on-time to the best other bay when the assigned bay is full; late
    only as last resort.  Prunes K by (committed w1*Z1+w3*Z3, -corridor).  Returns
    (objective,{bid:rec}) for the best feasible full realisation, or None.  env-gated,
    best-of candidate -> never-worse."""
    import time as _t, math as _mm
    B = prob_info["blocks"]; n = len(B); bays = prob_info["bays"]; m = len(bays)
    if n == 0 or m < 2:
        return None
    rel = [b["release_time"] for b in B]; pt = [b["processing_time"] for b in B]
    due = [b["due_date"] for b in B]
    prefs = [b["bay_preferences"] for b in B]; mxp = [max(p) for p in prefs]
    w = prob_info["weights"]; W1 = w["w1"]; W3 = w["w3"]
    def _amin(b):
        best = None
        for oi in range(len(B[b]["shape"])):
            bb = _orient_bbox(B[b], oi); a = (bb[2] - bb[0]) * (bb[3] - bb[1])
    ...
    try:
        ...
        return None
    RES = 12
    def _lfr(rects, Wd, Hd):
        if Wd <= 0 or Hd <= 0: return 0.0
        cw = Wd / RES; ch = Hd / RES
        occ = [bytearray(RES) for _ in range(RES)]
        for (x0, y0, x1, y1) in rects:
            ix0 = int(x0 / cw); ix1 = int((x1 - 1e-9) / cw); iy0 = int(y0 / ch); iy1 = int((y1 - 1e-9) / ch)
            if ix0 < 0: ix0 = 0
            if iy0 < 0: iy0 = 0
            if ix1 > RES - 1: ix1 = RES - 1
            if iy1 > RES - 1: iy1 = RES - 1
            for iy in range(iy0, iy1 + 1):
                row = occ[iy]
                for ix in range(ix0, ix1 + 1): row[ix] = 1
        hv = [0] * RES; bst = 0
        for iy in range(RES):
            row = occ[iy]
            for ix in range(RES): hv[ix] = 0 if row[ix] else hv[ix] + 1
            st = []
            for i in range(RES + 1):
                hh = hv[i] if i < RES else 0
                while st and hv[st[-1]] >= hh:
                    tp = st.pop(); a = hv[tp] * (i - (st[-1] if st else -1) - 1)
                    if a > bst: bst = a
                st.append(i)
        return bst * cw * ch
    def _ent(b):
        ...
        if hi > r:
            for k in range(1, 6): cs.add(r + ((hi - r) * k) // 6)
        return sorted(e for e in cs if e >= 0)
    _POSCAP = 24   # cap positions scored per (block,bay) to bound cost
    def _cands(b, bay, ontime, topm, pres_bay):
        bw = bays[bay]["width"]; bh = bays[bay]["height"]
        ens = [en for en in _ent(b) if not (ontime and en + pt[b] > due[b])]
        scored = []
        for en in ens:
            ex = en + pt[b]
            for oi in range(len(B[b]["shape"])):
                x0, y0, x1, y1 = _orient_bbox(B[b], oi)
                if x1 - x0 > bw or y1 - y0 > bh: continue
                cnt = 0
                for ix in range(_mm.ceil(-x0), _mm.floor(bw - x1) + 1):
                    for iy in range(_mm.ceil(-y0), _mm.floor(bh - y1) + 1):
                        if E.placement_feasible(bay, b, oi, float(ix), float(iy), en, ex):
                            nr = (ix + x0, iy + y0, ix + x1, iy + y1)
                            room = _lfr(pres_bay + [nr], bw, bh)
                            scored.append((room, (bay, oi, ix, iy, en, ex)))
                            cnt += 1
                            if cnt >= _POSCAP: break
                    if cnt >= _POSCAP: break
            if scored: break   # earliest feasible entry only (matches _spill_realize)
        scored.sort(key=lambda t: -t[0])
        return scored[:topm]
    order = sorted(range(n), key=lambda b: (rel[b], -area[b], due[b]))
    # state: dict(plc={bid:rec}, cost, room, presrects{bay:[bbox]})
    beam = [dict(plc={}, cost=0.0, room=0.0, presrects={j: [] for j in range(m)})]
    for b in order:
        if _t.time() > deadline:
            return None
        children = []
        _pl = pt[b]; _due = due[b]
        for st in beam:
            # load E with this state's placements that time-overlap b's window
            E.clear_all()
            _lo = rel[b]; _hi = max(rel[b], _due - _pl) + _pl
            for bb, rc in st["plc"].items():
                if rc["exit_time"] > _lo and rc["entry_time"] < _hi:
                    E.add(rc["bay_id"], bb, rc["orient_idx"], float(rc["x"]), float(rc["y"]),
                          rc["entry_time"], rc["exit_time"])
            cc = _cands(b, ext[b], True, M, st["presrects"][ext[b]])   # in-pref, top-M by room
            if not cc:                                                  # spill on-time
                for bay in sorted((j for j in range(m) if j != ext[b]), key=lambda j: -prefs[b][j]):
                    sp = _cands(b, bay, True, 1, st["presrects"][bay])
                    if sp: cc = sp; break
            if not cc:                                                  # late last resort
                for bay in [ext[b]] + sorted((j for j in range(m) if j != ext[b]), key=lambda j: -prefs[b][j]):
                    sp = _cands(b, bay, False, 1, st["presrects"][bay])
                    if sp: cc = sp; break
            if not cc:
                continue
            for room, (bay, oi, ix, iy, en, ex) in cc:
                x0, y0, x1, y1 = _orient_bbox(B[b], oi)
                npr = {j: list(v) for j, v in st["presrects"].items()}
                npr[bay].append((ix + x0, iy + y0, ix + x1, iy + y1))
                nplc = dict(st["plc"]); nplc[b] = {"block_id": b, "bay_id": bay, "x": ix, "y": iy,
                                                   "orient_idx": oi, "entry_time": en, "exit_time": ex}
                nc = st["cost"] + W3 * (mxp[b] - prefs[b][bay]) + W1 * max(0, ex - _due)
                children.append(dict(plc=nplc, cost=nc, room=st["room"] + room, presrects=npr))
        if not children:
            return None
        children.sort(key=lambda s: (s["cost"], -s["room"]))
        beam = children[:K]
    best = None
    for st in beam:
        if len(st["plc"]) != n: continue
        try:
            ck = check_feasibility(prob_info, _build_operations([st["plc"][b] for b in range(n)]))
        except Exception:
            continue
        if ck.get("feasible"):
            o = float(ck["objective"])
            if best is None or o < best[0]:
                best = (o, {b: dict(st["plc"][b]) for b in range(n)})
    return best
...
def _solve_once_impl(prob_info, timelimit=60, seed=12345,
    ...
    # --- Space-time 3DTCS construction (env ST3DTCS; half the workers).  Full-scan
    # placement scored to maximise 3D-total-contiguous-surface contact in (x,y,t)
    # -> much tighter space-time packing than bottom-left (measured -10..-24%
    # construction vs the engine on dense tardiness instances, always crane-
    # feasible).  Adaptive step-down keeps it within a generous construction budget;
    ...
    if _st3_on:
        try:
            _st_dl = start + (deadline - start) * 0.62
            _stst = _st_construct_cpp(prob_info, candidate_orders[0], _st_dl, step=1)
            placed = set(_stst.assign.keys())
            for bid in range(n_blocks):
                if bid not in placed:
                    _try_place_block(_stst, bid, list(range(n_bays)), deadline)
            if len(_stst.assign) == n_blocks:
                _sol = _build_operations(list(_stst.assign.values()))
                _chk = check_feasibility(prob_info, _sol)
                if _chk["feasible"] and _chk["objective"] < best_state_obj:
                    best_state = _stst
                    best_state_obj = _chk["objective"]
                    first_complete = _stst
        except Exception:
            pass
...
def _smallright_construct(prob_info, deadline_s, small_thresh=0.60, step=1, mode="flatbl", order="rank", ext_entry=None, tiebreak="rank", ext_bay=None, feat_w=None):
    ...
    if feat_w is not None:
        ...
        # zlay: number of Z-LAYERS (crane-relevant vertical height, orientation-invariant).
        # Idea2: a tall (many-layer) block descending into an EMPTY floor blocks nothing;
        # placed LAST it blocks everyone below its descent -> descend TALL-first within a
        # bay's coexisting set to minimise crane spill.  (Distinct from "hgt" = y-footprint.)
        _zlay=[len(B[b]["shape"][0]["layers"]) for b in range(n)]
        # crane-criticality: "most crane-constrained first".  critA = footprint area x
        # layers (harder to find a clear drop-zone).  critC = preferred-bay CONGESTION:
        # total area of OTHER blocks that prefer the same bay AND overlap in time -> the
        # direct spill-risk measure (grab the spot before the bay peaks).
        _critA=[ar[b]*_zlay[b] for b in range(n)]
        _pbay=[max(range(m), key=lambda j: B[b]["bay_preferences"][j]) for b in range(n)]
        _critC=[0.0]*n
        for _bb in range(n):
            _jb=_pbay[_bb]; _rb=rel[_bb]; _db=due[_bb]; _s=0.0
            for _cc in range(n):
                if _cc!=_bb and _pbay[_cc]==_jb and rel[_cc]<_db and _rb<due[_cc]:
                    _s+=ar[_cc]
            _critC[_bb]=_s
        _F={
            ...
            "zlay":_mkr(_zlay,True), "critA":_mkr(_critA,True), "critC":_mkr(_critC,True),
        }
    ...
    # DESCENT-CORRIDOR room: largest empty rectangle over a coarse RES x RES grid ==
    # biggest contiguous vertical drop-zone still open for future crane descents.  The
    # true binding limit is crane accessibility, not area, so scoring a placement by the
    # corridor it LEAVES (not just where it sits) is the crane-native placement rule that
    # bottom-left/area modes miss.  Used by the "skyline" place_custom mode and the beam.
    _RES=16
    def _occ_grid(rects, W, H):
        # occupancy bytearray grid + cell size; built ONCE per (bay, time) so the
        # LER of many candidate positions reuses it (tcp speed path).
        cw=W/_RES; ch=H/_RES
        occ=[bytearray(_RES) for _ in range(_RES)]
        for (x0,y0,x1,y1) in rects:
            ix0=int(x0/cw); ix1=int((x1-1e-9)/cw); iy0=int(y0/ch); iy1=int((y1-1e-9)/ch)
            if ix0<0: ix0=0
            if iy0<0: iy0=0
            if ix1>_RES-1: ix1=_RES-1
            if iy1>_RES-1: iy1=_RES-1
            for iy in range(iy0,iy1+1):
                row=occ[iy]
                for ix in range(ix0,ix1+1): row[ix]=1
        return occ, cw, ch
    def _ler_of(occ, cw, ch):
        hv=[0]*_RES; best=0
        for iy in range(_RES):
            row=occ[iy]
            for ix in range(_RES): hv[ix]=0 if row[ix] else hv[ix]+1
            stack=[]
            for i in range(_RES+1):
                hh=hv[i] if i<_RES else 0
                while stack and hv[stack[-1]]>=hh:
                    top=stack.pop(); hgt=hv[top]; left=stack[-1] if stack else -1
                    a=hgt*(i-left-1)
                    if a>best: best=a
                stack.append(i)
        return best*cw*ch
    def _lfr(rects, W, H):
        ...
        occ,cw,ch=_occ_grid(rects, W, H)
        return _ler_of(occ, cw, ch)
    BAND=0.6
    ...
    def place_custom(b,cur,is_small,windows=None,topm=None):
        ...
        for j in _bl_b:
            ...
            occ_base=None
            _tcp_occ=None; _tcp_cw=0.0; _tcp_ch=0.0; _tcp_memo=None   # tcp LER cache (per b,j)
            for oi in range(len(B[b]["shape"])):
                        ...
                        if E.placement_feasible(j,b,oi,float(ix),float(iy),cur,ex):
                            ...
                            if mode in ("bigbottom","cornerBL","cornerBR","cornerTL","cornerTR"):
                                ...
                                if mode=="coreperi":
                                    ...
                                    # (max empty rectangle after placing here), so future
                                    # crane drops aren't blocked -> fewer forced spills.
                                    # skypref: preferred-bay first, then corridor (P3 Z3).
                                    _rl=[bb for (bb,_e) in present_by_bay[j]]
                                    _rl.append((wx,wy,wx+w,wy+h))
                                    _room=_lfr(_rl, bw_j, bh_j)
                                    if mode=="skypref":
                                        sc=(_mxp[b]-B[b]["bay_preferences"][j], -_room, wy, wx)
                                    else:
                                        sc=(-_room, wy, wx, j)
                                elif mode=="cluster":
                                    # CLUSTER-EXPANSION (군집확장): grow one compact mass by
                                    # MAXIMISING contact (shared edge length) with already-
                                    # placed block bboxes + bay walls, instead of bottom-left.
                                    # A single tight cluster leaves ONE large contiguous free
                                    # region -> unobstructed crane descent corridors -> more
                                    # blocks seat on-time (the crane, not area, is the binding
                                    # limit).  Tiebreak bottom-left.  best-of variant (never
                                    # sole rule) so it can't regress.
                                    cx0=wx; cy0=wy; cx1=wx+w; cy1=wy+h; _ct=0.0
                                    if cx0<=1e-9: _ct+=h
                                    if cx1>=bw_j-1e-9: _ct+=h
                                    if cy0<=1e-9: _ct+=w
                                    if cy1>=bh_j-1e-9: _ct+=w
                                    for (bx0,by0,bx1,by1),_pex in present_by_bay[j]:
                                        if cx1<=bx0-1e-9 or cx0>=bx1+1e-9 or cy1<=by0-1e-9 or cy0>=by1+1e-9:
                                            if abs(cx1-bx0)<1e-9 or abs(cx0-bx1)<1e-9:
                                                _ov=min(cy1,by1)-max(cy0,by0)
                                                if _ov>0: _ct+=_ov
                                            if abs(cy1-by0)<1e-9 or abs(cy0-by1)<1e-9:
                                                _ov=min(cx1,bx1)-max(cx0,bx0)
                                                if _ov>0: _ct+=_ov
                                    sc=(-_ct, wy, wx, j)
                                elif mode=="tcp":
                                    # TEMPORAL CORRIDOR PRESERVATION (hypothesis 1):
                                    # generalises coreperi WITHOUT a ratio gate.  Score =
                                    # DURATION-weighted fragmentation of the largest empty
                                    # rectangle (= crane descent corridor).  Two fixes over
                                    # the (dead, losing) skyline mode: (a) count only rects
                                    # ACTIVE at this block's entry (skyline was time-blind),
                                    # (b) weight the corridor loss by pt[b] (stay length).
                                    # A long-stay block heavily avoids shrinking the corridor
                                    # (=> periphery, exactly coreperi's parker rule); a short-
                                    # stay one barely weights it (=> bottom-left).  Self-adapts
                                    # to density: low contention -> corridor huge -> term flat
                                    # -> bottom-left dominates; high contention -> compacts.
                                    # FAST LER: build the present-occupancy grid ONCE per
                                    # (b,j), then overlay each candidate and memoise the LER
                                    # by its rasterised cell-box (many fine positions collapse
                                    # to the same 16x16 cells) -> ~10-50x fewer LER computes.
                                    if _tcp_occ is None:
                                    ...
                                    if _room is None:
                                        ...
                                        _room=_ler_of(_tmp, _tcp_cw, _tcp_ch)
                                        _tcp_memo[_mk]=_room
                                    _A=bw_j*bh_j
                                    # DURATION-weighted corridor-loss: a long-stay block is
                                    # driven to the position that leaves the largest empty
                                    # rectangle (crane corridor), because it fragments space
                                    # for its whole stay -- this recovers coreperi's periphery
                                    # placement from a single principled score, gate-free and
                                    # self-adapting to density (low contention -> corridor
                                    # ~flat -> bottom-left dominates).  Wins the contended
                                    # (P5) regime; at saturation (ratio>1) the corridor signal
                                    # is noise and pure compaction wins -> keep tcp a best-of
                                    # variant, never the sole rule (like bigleft/coreperi).
                                    # QUANTISE the corridor-loss fraction to _TCPL levels:
                                    # at saturation every position leaves a ~equally tiny
                                    # corridor so they collapse to one level -> term flat ->
                                    # compaction tiebreaker wins; at contention corridors
                                    # span many levels -> drives periphery.  Self-adapts to
                                    # density with NO ratio gate.
        ...
        return best
    # ---- MINIMAL INTERNAL BEAM (validation, env BEAMK>0; byte-identical off) ----
    # Keeps K partial packings; at each block-decision expands the top-M place_custom
    # positions and prunes to K by committed cost (w1*tardiness + w3*pref).  Reuses the
    # exact place_custom scoring + E.placement_feasible; each state swaps E via
    # clear_all+re-add so feasibility is faithful.  One block placed per expansion so
    # all live states stay at equal depth -> cost comparison is fair.
    _BEAMK = int(os.environ.get("BEAMK","0"))
    if _BEAMK > 0:
        _BEAMM = int(os.environ.get("BEAMM","2"))
        _w=prob_info["weights"]; _w1=_w["w1"]; _w3=_w["w3"]
        _orl=sorted(range(n),key=lambda b:rel[b])
        _prefs=[B[b]["bay_preferences"] for b in range(n)]
        def _isS(b): return ra[b]>=small_thresh
        _SKY = os.environ.get("SKY","0")=="1"   # descent-corridor room as beam value
        _init=dict(plc=[], pend=set(), ri=0, cur=(min(rel) if n else 0),
                   cost=0.0, room=0.0, np=0, done=False)
        def _expand(st):
            _plc=st["plc"]
            def _sync(cur):
                # feasibility at `cur` depends only on blocks still present (ex>cur);
                # exited blocks are irrelevant -> add just those -> O(concurrent), not O(all)
                E.clear_all()
                for j in range(m): present_by_bay[j]=[]
                for (b,j,oi,ix,iy,en,ex) in _plc:
                    if ex>cur:
                        E.add(j,b,oi,float(ix),float(iy),en,ex)
                        x0,y0,x1,y1=bbox(b,oi); present_by_bay[j].append(((ix+x0,iy+y0,ix+x1,iy+y1),ex))
            pend=set(st["pend"]); ri=st["ri"]; cur=st["cur"]; chosen=None; cands=None
            _sync(cur)
            while True:
                while ri<n and rel[_orl[ri]]<=cur: pend.add(_orl[ri]); ri+=1
                for b in sorted(pend,key=key):
                    cc=place_custom(b,cur,_isS(b),topm=_BEAMM)
                    if cc: chosen=b; cands=[(j,oi,ix,iy,cur,cur+pt[b]) for (j,oi,ix,iy) in cc]; break
                if chosen is not None: break
                c=[]
                if ri<n: c.append(rel[_orl[ri]])
                _nx=[ex for j in range(m) for (bb,ex) in present_by_bay[j]]
                if _nx: c.append(min(_nx))
                if not c:
                    if pend:  # stuck: force-place lowest-key via fallback
                        b=sorted(pend,key=key)[0]
                        try: r=E.find_best_placement(b,bay_list,[cur])
                        except Exception: r=None
                        if r and r[0]:
                            chosen=b; cands=[(int(r[1]),int(r[2]),int(r[3]),int(r[4]),int(r[5]),int(r[6]))]
                            break
                    st2=dict(st); st2["cur"]=cur; st2["done"]=True; return [st2]
                cur=min(c); _sync(cur)
            out=[]
            _baserects={jj:[bb for (bb,_e) in present_by_bay[jj]] for jj in range(m)} if _SKY else None
            for (j,oi,ix,iy,en,ex) in cands:
                _rm=st["room"]
                if _SKY:
                    x0b,y0b,x1b,y1b=bbox(chosen,oi); _nr=(ix+x0b,iy+y0b,ix+x1b,iy+y1b)
                    _rm=0.0
                    for jj in range(m):
                        _rc=_baserects[jj] if jj!=j else _baserects[jj]+[_nr]
                        _rm+=_lfr(_rc, bays[jj]["width"], bays[jj]["height"])
                child=dict(plc=st["plc"]+[(chosen,j,oi,ix,iy,en,ex)],
                           pend=pend-{chosen}, ri=ri, cur=cur,
                           cost=st["cost"]+_w1*max(0,ex-due[chosen])+_w3*(_mxp[chosen]-_prefs[chosen][j]),
                           room=_rm, np=st["np"]+1, done=(st["np"]+1>=n))
                out.append(child)
            return out
        beam=[_init]; _bt0=_t.time()
        _skey=(lambda s:(s["cost"], -s["room"], -s["np"])) if _SKY else (lambda s:(s["cost"], -s["np"]))
        while any(not s["done"] for s in beam):
            if _t.time()-_bt0>deadline_s: break
            ch=[]
            for st in beam:
                if st["done"]: ch.append(st); continue
                ch.extend(_expand(st))
            if not ch: break
            ch.sort(key=_skey)
            beam=ch[:_BEAMK]
        best=min(beam,key=_skey)
        return {b:{"block_id":b,"bay_id":j,"x":ix,"y":iy,"orient_idx":oi,
                   "entry_time":en,"exit_time":ex} for (b,j,oi,ix,iy,en,ex) in best["plc"]}
    ...
    # ADAPTSTEP (variance reducer): if a step=1 construction falls behind schedule it
    # would break INCOMPLETE at the deadline -> discarded -> best-of falls to a coarse
    # step=2 worker (the bad basin, e.g. prob_38 34.5M vs 37.8M).  When enabled, on a
    # step=1 build that is behind pace, coarsen the REMAINING blocks to step=2 so the
    # build COMPLETES (mostly-tight prefix + coarse tail) instead of dying incomplete.
    # No-op when step!=1 or on schedule -> the good basin stays byte-identical.
    ...
    while True:
        ...
        # ADAPTSTEP: behind pace (>=70% time gone, <85% of blocks placed) -> coarsen the
        # remaining scan to step=2 once so the build finishes instead of dying incomplete.
...
def algorithm(prob_info, timelimit=60):
    ...
    # FEWWORK (env research): on LARGE high-density instances step=1 (the tight,
    # low-tardiness pack) needs ~all of the 60s and, under 4-way CPU/memory-bandwidth
    # contention, often just misses the deadline (prob_38 bimodal 34.5M/37.8M).
    # Running FEWER workers there cuts contention so step=1 completes reliably.  Only
    # applies to large high-density; other instances keep 4 workers.  NFEW sets the
    # count (default 2).  FEWWORK=0 (default) keeps the shipped 4-worker path.
    try:
