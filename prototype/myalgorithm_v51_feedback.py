...
def _exact_reassign(prob_info, bay_unit, deadline, mip_cap=6.0):
    ...
    try:
        ...
        for _bit in range(6 if _spill_on else 5):
            ...
            if _spill_on:
                try:
                    ...
                    if _spb is not None:
                        ...
                        # ROUND 0 always falls through to the _smallright realisation below
                        # so the seed-driven Benders path (which sets the SA-repair seed and
                        # wins on some instances, e.g. prob_3) is never skipped by feedback.
                except Exception:
                    pass
            _rt0 = _t.time()
        ...
        if os.environ.get("EXACT", "1") == "1" and time.time() < deadline - 6.0:
            try:
                _res = _exact_reassign(prob_info, bay_unit,
                                       min(deadline - 2.0, time.time() + 16.0))
                if _res is not None:
