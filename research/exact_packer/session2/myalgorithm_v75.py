...
def _vlns_refine(prob_info, assign, bay_unit, deadline, seed=7):
    """Pure-C++ cranepack VLNS refiner (cranepack.refine): the ENTIRE low-density SLS --
    descent (FREE~10 relocator, multi-entry, fine grid) + window-repack + ruin-recreate
    kicks + SA on the Z2+Z3 objective -- runs in C++ with no per-iteration Python
    overhead (~10-50x more iterations than the Python loop).  One-time geometry marshal,
    one refine() call, one final grader feasibility check as a correctness guard: the
    C++ best is kept only if the grader agrees it is feasible and not worse than the
    input (so the pipeline is never-worse); returns `assign` untouched if cranepack is
    absent.  Low-density (Z1=0): objective = w2*Z2 + w3*Z3."""
    import time as _t
    CP = _load_cranepack()
    if CP is None:
        return assign
    try:
        import numpy as _np
        B = prob_info["blocks"]; n = len(B); bays = prob_info["bays"]; m = len(bays)
        if n == 0 or m < 2 or len(assign) != n:
            return assign
        w = prob_info["weights"]; w2 = float(w["w2"]); w3 = float(w["w3"])
        budget = deadline - _t.time()
        if budget < 1.0:
            return assign
        blocks_in = []
        for b in range(n):
            ol = [[_np.ascontiguousarray(_np.asarray(L, dtype=_np.float64))
                   for L in Block(block_id=b, block_data=B[b], x=0, y=0, orient_idx=o).layers_at_pos()]
                  for o in range(len(B[b]["shape"]))]
            obb = [tuple(float(v) for v in _orient_bbox(B[b], o)) for o in range(len(B[b]["shape"]))]
            blocks_in.append((ol, obb, int(B[b]["release_time"]), int(B[b]["processing_time"]),
                              int(B[b]["due_date"]), [float(p) for p in B[b]["bay_preferences"]],
                              float(B[b]["workload"])))
        baydims = [(float(bb["width"]), float(bb["height"])) for bb in bays]
        bayunit = [float(x) for x in bay_unit]
        init = [(int(assign[b]["bay_id"]), int(assign[b]["orient_idx"]), int(assign[b]["x"]),
                 int(assign[b]["y"]), int(assign[b]["entry_time"]), int(assign[b]["exit_time"]))
                for b in range(n)]
        _bo, outA, _iters = CP.refine(blocks_in, baydims, bayunit, w2, w3, init, budget,
                                      seed=seed, WIN=6, NPULL=2, STEP=8)
        cand = {b: {"block_id": b, "bay_id": int(outA[b][0]), "orient_idx": int(outA[b][1]),
                    "x": int(outA[b][2]), "y": int(outA[b][3]), "entry_time": int(outA[b][4]),
                    "exit_time": int(outA[b][5])} for b in range(n)}
        ck = check_feasibility(prob_info, _build_operations(list(cand.values())))
        if ck["feasible"]:
            bck = check_feasibility(prob_info, _build_operations(list(assign.values())))
            if (not bck["feasible"]) or ck["objective"] <= bck["objective"] + 1e-6:
                return cand
        return assign
    except Exception:
        if os.environ.get("Z3DBG") == "1":
...
def _streamlined_lowdensity(prob_info, bay_unit, deadline, rng, worker_id, use_cpp=True):
    ...
    try:
        res = _exact_reassign(prob_info, bay_unit,
                              min(deadline - 3.0, T0 + (0.35 if HAVE_CRANEPACK else 0.60) * total),
    ...
    if HAVE_CRANEPACK:
        ...
        try:
            ...
            av = _vlns_refine(prob_info, {b: dict(v) for b, v in best_a.items()},
                              bay_unit, deadline - 0.5, seed=_vseed)
            ov = _score(av)
            if ov < best_obj:
...
def _solve_once_impl(prob_info, timelimit=60, seed=12345,
    ...
    if _st3_on and _temporal_os(prob_info) >= float(os.environ.get("ST3TOS", "0.30")):
        # DENSITY FAST-PATH: fire directly, full budget (historical v62/v63 behavior).
        try:
            _st_dl = start + (deadline - start) * 0.62
            if os.environ.get("BRKGA", "1") == "1":
                ...
                for bid in range(n_blocks):
                    if bid not in _stst.assign:
                        _try_place_block(_stst, bid, list(range(n_bays)), deadline)
                if len(_stst.assign) == n_blocks:
                    _chk = check_feasibility(prob_info, _build_operations(list(_stst.assign.values())))
                    if _chk["feasible"] and _chk["objective"] < best_state_obj:
                        best_state = _stst; best_state_obj = _chk["objective"]; first_complete = _stst
        except Exception:
            pass
    elif _st3_on:
    ...
    if hybrid_flag or coreperi_flag:
        ...
        try:
            ...
            def _try_smallright():
                ...
                # coreperi (long-stay big blocks -> periphery) is offered gate-free as a
                # best-of TAIL: best-of keeps it where it wins (a hidden P5 at ANY ratio),
                # ignores it elsewhere, and the numba guard is preserved everywhere.  This
                # replaced the old dedicated coreperi WORKER, which was gated to a
                # single-training-instance (prob_24) [0.60,0.70) ratio fingerprint -- that
                # dedicated worker and its p5_band gate have now been removed entirely.
    ...
    try:
        with multiprocessing.Manager() as manager:
            ...
            try:
                ...
                _ratio_val = 0.0
                _hi_ratio = False
            # The old p5_band = [0.60,0.70) ratio window was a single-training-instance
            # (prob_24) fingerprint that dedicated the numba-guard slot to a heavy
            # coreperi worker.  It was retired (coreperi now runs gate-free as a best-of
            # tail, the numba guard is preserved everywhere) so p5_band is always False.
            _p5_band = False
            ...
            # coreperi_flag (15th arg) is now always False -- the dedicated coreperi
            # worker was retired; coreperi survives only as a gate-free best-of tail.
