            ...
            try:
                if _anchor is not None:
                    _ob, _flat = E.contact_beam(order_ids, areas_l, wl, int(B), int(K), int(step),
                                                ...
                                                float(_sc), float(swy), float(swx), float(cohort), float(shadow), float(span), int(lex), float(shadoww), float(conw), float(span2))
                else:
                    _ob, _flat = E.contact_beam(order_ids, areas_l, wl, int(B), int(K), int(step),
                                                float(pos_lam), float(prefw), float(mu),
                                                float(w1), float(w2), float(w3_route), float(fut_beta),
    ...
    for step, frac in ((1, 0.6), (2, 1.0)):
        ...
        try:
            r = _contact_beam(prob_info, left, B=(1 if cfg.get("lex") else _beam_width(cfg["Bmul"])),
                              ...
                              w3mul=cfg["w3mul"], mum=cfg.get("mum", 1.0), cohort=cfg.get("cohort", 0.0), shadow=cfg.get("shadow", 0.0), span=cfg.get("span", 0.0), lex=cfg.get("lex", 0.0), shadoww=cfg.get("shadoww", 0.0), span2=cfg.get("span2", 0.0), conw=cfg.get("conw", 1.0), swy=cfg.get("swy", 1.0), swx=cfg.get("swx", 0.01), step=step)
        except Exception:
            r = None
        if r:
            s = _recs_to_ops(r, n)
...
#             crane descents.
_AXES = [
    dict(Bmul=1.0, K=4, pos_lam=0.10, order="defer_big", fut_beta=1.0, prefw=0.0, w3mul=1.0, cohort=0.0, span2=0.0, conw=0.0, dk=0),
    dict(Bmul=1.0, K=4, pos_lam=0.12, order="defer_big", fut_beta=1.0, prefw=0.0, w3mul=3.0, cohort=0.3, span2=1.0, conw=1.0, dk=0),
    dict(Bmul=0.7, K=5, pos_lam=0.15, order="lst",       fut_beta=0.0, prefw=0.0, w3mul=3.0, cohort=0.3, span2=1.0, conw=0.25, dk=0),
    dict(Bmul=0.7, K=5, pos_lam=0.05, order="edd",       fut_beta=1.5, prefw=0.0, w3mul=1.0, cohort=0.3, span2=0.0, conw=0.0, dk=0),
    dict(Bmul=1.4, K=3, pos_lam=0.10, order="big_first", fut_beta=0.5, prefw=0.0, w3mul=6.0, cohort=0.3, span2=1.0, conw=1.0, dk=0),
    dict(Bmul=0.5, K=6, pos_lam=0.20, order="defer_big", fut_beta=0.0, prefw=0.0, w3mul=1.5, cohort=0.0, span2=1.0, conw=0.25, dk=0),
]
...
def _regrow(prob_info, sol, budget, cfg, stay, share=1.0, anchor=None, mum=1.0):
    ...
    try:
        r = _contact_beam(prob_info, budget, B=B, K=cfg["K"], pos_lam=cfg["pos_lam"],
                          ...
                          mum=mum, cohort=cfg.get("cohort", 0.0), shadow=cfg.get("shadow", 0.0), span=cfg.get("span", 0.0), lex=cfg.get("lex", 0.0), shadoww=cfg.get("shadoww", 0.0), span2=cfg.get("span2", 0.0), conw=cfg.get("conw", 1.0), swy=cfg.get("swy", 1.0), swx=cfg.get("swx", 0.01))
    except Exception:
        return None
