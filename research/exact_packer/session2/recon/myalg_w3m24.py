    ...
    for step, frac in ((1, 0.6), (2, 1.0)):
        ...
        try:
            r = _contact_beam(prob_info, left, B=(1 if cfg.get("lex") else _beam_width(cfg["Bmul"])),
                              ...
                              w3mul=cfg["w3mul"], mum=cfg.get("mum", 1.0), cohort=cfg.get("cohort", 0.0), shadow=cfg.get("shadow", 0.0), span=cfg.get("span", 0.0), lex=cfg.get("lex", 0.0), shadoww=cfg.get("shadoww", 0.0), conw=cfg.get("conw", 1.0), step=step)
        except Exception:
            r = None
        if r:
            s = _recs_to_ops(r, n)
...
#             crane descents.
_AXES = [
    dict(Bmul=1.0, K=4, pos_lam=0.10, order="defer_big", fut_beta=1.0, prefw=0.0, w3mul=24.0, cohort=0.0, conw=0.25, dk=0),
    dict(Bmul=1.0, K=4, pos_lam=0.12, order="defer_big", fut_beta=1.0, prefw=0.0, w3mul=24.0, cohort=0.3, conw=0.25, dk=0),
    dict(Bmul=0.7, K=5, pos_lam=0.15, order="lst",       fut_beta=0.0, prefw=0.0, w3mul=24.0, cohort=0.3, conw=0.25, dk=0),
    dict(Bmul=0.7, K=5, pos_lam=0.05, order="edd",       fut_beta=1.5, prefw=0.0, w3mul=24.0, cohort=0.3, conw=0.25, dk=0),
    dict(Bmul=1.4, K=3, pos_lam=0.10, order="big_first", fut_beta=0.5, prefw=0.0, w3mul=24.0, cohort=0.3, conw=0.25, dk=0),
    dict(Bmul=0.5, K=6, pos_lam=0.20, order="defer_big", fut_beta=0.0, prefw=0.0, w3mul=24.0, cohort=0.0, conw=0.25, dk=0),
]
