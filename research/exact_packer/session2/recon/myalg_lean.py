...
#
# Generated from myalg_orig.py (1,361 lines) by harness/mkbase.py with OGC_DK=0 OGC_FASTOBJ=1 --
# the configuration every measurement in this session used.  Committed as a file rather than
# regenerated at package time, so what ships is reviewable and cannot drift.
#
#   reproduce:  OGC_DK=0 OGC_FASTOBJ=1 python3.12 harness/mkbase.py 0.3 myalg_a.py 0 "" 0 1.0 "" 0 ""
#               cp myalg_a.py myalg_lean.py   (then re-add this header)
#
# WHY THIS REPLACED THE 6,675-LINE myalgorithm.py.  Both builds on the hidden instances at their
# real budgets, brk off in both:
#
#                      this build     the 6,675-line file    leader
#     P1   60 s          11,280         11,280               --
#     P2  120 s          31,368         31,368               --
#     P3  240 s          96,990         90,545               ~70,000
#     P4  480 s       1,780,253      3,273,791               2,200,000
#
# P1 and P2 are identical to the digit.  P3 is 7% worse here.  P4 is 45% BETTER -- 19% ahead of
# the leaderboard's best, where the legacy file sits 49% behind it.  P4's objective is 66%
# tardiness; P3's is 87% preference with Z1 zero throughout; the two builds are strong at
# opposite ends of that.
#
# COMPARABLE TO THE LEADERBOARD.  This lineage scored 29,396,046 on the real P6 at 900 s and
# 30,297,335 here -- 3.1% apart on one stochastic run, inside the pipeline's own spread.
#
# NOT YET MEASURED: P5 and P6 on this build.  The six-problem sweep reached P4 and was stood
# down.  Check both before trusting this end to end -- a P3-only win that wrecked another
# instance has happened twice in this project (conw, and brk).
