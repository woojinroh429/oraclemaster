...
def algorithm(prob_info, timelimit=60):
    ...
    try:
        ...
        _shdir = None
    # TWO-PHASE AIM RACE (OGC_RACE=1).  Stop paying for the wrong half of the portfolio.
    #
    # Each worker's beam aim is fixed at fork by _aims[wid % 2] = 0.90 / 0.10, and the C++ reads
    # OGC_BEAMAIM once per process into a static, so a live worker can NEVER change it.  That
    # matters because the aim decides the run on some instances and it decides it by a lot:
    #
    #     P16   aim 0.10 wins, the other half is 33-38% behind
    #     P36   aim 0.10 wins, 17% behind at every one of six budgets
    #     P34   aim 0.90 wins, 17% behind
    #
    # Both are 300-block instances, so size does not predict it, and nothing readable off an
    # instance has ever separated them -- which is exactly why the portfolio exists.  But the
    # consequence is that half the cores spend the whole budget on a draw that was never going to
    # be the minimum, every run.
    #
    # Redrawing those workers in place was built and measured first, and it does not work: a
    # worker restarted at 60% has 40% of the depth and cannot beat its own earlier deep draw --
    # on P16 workers 0 and 2 came back with their pre-redraw values to the digit, twice each.
    # Depth is not free, which is the same wall OGC_ROUNDS hit.
    #
    # A new PROCESS can pick the aim, though.  So: spend a short first phase finding out which
    # aim is ahead on this instance -- the signal is already unambiguous at 31% of budget -- then
    # spend the rest with every worker on the winner.  Against the shipped build that trades a
    # little depth (phase two is 1 - OGC_RACEF of the budget) for twice as many draws on the
    # right side of a 17-38% gap, and phase one's draws are kept as well, so the minimum is over
    # 2*nw draws rather than nw.
    #
    # It cannot lose the incumbent: `best` spans both phases.  Whether the depth it costs is
    # cheaper than the wasted half it recovers is the measurement, and it is not obvious --
    # a 4-draw minimum at 75% depth against a 2-draw minimum at 100% depth is close enough
    # arithmetic that it has to be run rather than argued.
    _RACE = os.environ.get("OGC_RACE") == "1" and nw > 1
    if _RACE:
        _f1 = float(os.environ.get("OGC_RACEF", "0.25"))
        _aimset = [a.strip() for a in os.environ.get("OGC_AIMSET", "0.90,0.10").split(",") if a.strip()]
        _b1 = max(4.0, wbudget * _f1)
        _o1 = _pool_round(prob_info, _b1, 0, nw, cwd, _shdir,
                          timelimit - (time.time() - t0) - 1.0)
        _bya = {}
        for _i, _s in enumerate(_o1):
            if _s is None:
                continue
            _v, _ = _total(prob_info, _s)
            if _v < best[0]:
                best = (_v, _s)
            _a = _aimset[_i % len(_aimset)]
            if _v < _bya.get(_a, float("inf")):
                _bya[_a] = _v
        if _bya:
            _win = min(_bya, key=lambda a: _bya[a])
            os.environ["OGC_AIMSET"] = _win          # inherited at fork by every phase-2 worker
            if os.environ.get("OGC_WSTAT"):
                import sys as _sy
                _sy.stderr.write("RACE phase1 %s -> aim %s\n"
        ...
        for _s in _o2:
            ...
            if _v < best[0]:
                best = (_v, _s)
        _R = 0                                        # the race replaces the round loop entirely
    for _r in range(_R):
