...
def _worker(args):
    ...
    # OGC_W3GRID: replace the w3mul each axis carries, positionally, leaving everything else alone.
    #
    # w3mul is how hard the beam's rank routes toward preferred bays -- a bias multiplier on top of
    # the instance's true w3, correcting the greedy lookahead's undervaluation of preference (a
    # non-preferred bay is a permanent loss; tardiness can sometimes be recovered).  The shipped
    # values are 1.0, 3.0, 3.0, 1.0, 6.0, 1.5 and have not changed since 08-02, which is before the
    # final set was identified as a different problem on 08-03.
    #
    # Two measurements say that grid is aimed at the wrong distribution.  First, the exchange rate
    # w1/w3 -- how many units of preference one unit of tardiness costs -- has median 84.5 on the
    # preliminary set and 23.3 on the final one, a factor of 3.6, with twenty of the forty final
    # instances below 20x where the preliminary lower quartile was 44x.  Preference is worth far
    # more than the grid was tuned for.  Second, and more sharply: with nw = 4 the live wids are
    # 0..3, so only axes 0..3 ever OPEN a run, and their w3mul values are 1.0, 3.0, 3.0, 1.0 --
    # two distinct values.  The widest entry, 6.0, sits on axis 4 and never opens.
    #
    # Positional replacement rather than a scale factor, so an arm can widen the opening spread
    # without also moving the reserve slots, and the two are separable.  Empty or unset changes
    # nothing.
    _w3g = os.environ.get("OGC_W3GRID")
    if _w3g:
        try:
            _vals = [float(x) for x in _w3g.split(",") if x.strip()]
            if _vals:
                # INDEX BY THE ORIGINAL AXIS, NOT BY POSITION IN THE ROTATION.
                #
                # `axes` is already rotated per worker at this point --
                # axes = [_AXES[(wid + i) % len(_AXES)] ...] -- so a positional assignment gives
                # EVERY worker _vals[0] as its opening multiplier, which is the exact opposite of
                # spreading the opening values and would have made the arm meaningless.  Mapping
                # rotation position back to the original axis index keeps the property the
                # experiment is about: worker wid opens on axis wid % 6 and therefore on that
                # axis's multiplier.
                axes = [dict(_c, w3mul=_vals[((wid + _i) % len(_AXES)) % len(_vals)])
                        for _i, _c in enumerate(axes)]
        except Exception:
            pass
    try:
