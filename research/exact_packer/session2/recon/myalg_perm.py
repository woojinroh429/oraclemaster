...
def _worker(args):
    ...
    # OGC_DK=<k>: SEARCH OVER PERMUTATIONS INSTEAD OF USING ONE.
    #
    # The C++ beam takes `order` and places order[level] in EVERY state, so all states at level k
    # hold the same block SET and differ only in placements.  ogc_fast.cpp says so itself, beside
    # the order-independent dedup it ships: "MEASURED INERT ... our beam uses a FIXED dispatch
    # order ... The reference needs this because its orders vary."  That dedup is dead code for us
    # precisely because we never vary the order, and live for whatever it was compared against.
    #
    # The permutation is the biggest lever measured on this problem: cdecomp put the construction
    # spread across orders at 32-210%, against roughly 2% for the combined range of everything
    # else this session touched -- budget policy, axis sets, the w3mul grid, brk removal.  A rival
    # reaching 2.40M on hidden prob_1 against our best-of-five 2,847,060 is 18.6%, which is
    # permutation scale rather than tuning scale.
    #
    # _draw_order already samples a permutation: repeatedly take a uniform pick from the top-k of
    # what remains under the axis's own priority.  k=1 reproduces the fixed order exactly, k>1
    # widens the window.  _beam_once calls it whenever cfg["dk"] > 1, and all six shipped axes
    # carry dk=0 -- so it has never run.  The operator loop calls the beam many times per worker,
    # so each call drawing a fresh permutation turns a run into a best-of-N over permutations at
    # no extra cost.  That is the cheap way to find out whether the invasive C++ change (letting
    # each beam STATE choose its own next block, which is what makes the dedup live) is worth it.
    _dk = os.environ.get("OGC_DK")
    if _dk:
        try:
            _k = int(_dk)
            if _k > 1:
                axes = [dict(_c, dk=_k) for _c in axes]
        except Exception:
            pass
    try:
