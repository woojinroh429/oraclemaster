...
struct Engine {
    ...
    // CONTACT-MAXIMISING position picker (Phase2 sc + Phase3 d_rank) against the
    ...
    // sorted by d_rank -- the fast core of the beam's candidate stage.
    // ALTERNATIVE position scorer (env OGC_OURSCORE=1), in place of the raw -contact above:
    //  (1) contact-DENSITY = contact / block-bbox-half-perimeter -> rewards how WELL-NESTED a
    //      block is for ITS OWN size, removing the bias toward big blocks with long
    ...
    // Greedy event-driven construction using best_cell_contact (contact-max Phase2/3).
    ...
    // GUIDED-RECONSTRUCTION anchor (a rung_G-style anchor, different mechanism):
    ...
    // W2 IN THE CANDIDATE RANK (the one term the rank was missing).
    // A candidate rank over the full objective delta
    ...
    void best_cell_contact_tl(const std::vector<std::vector<Placed>>& TL,int bid,int cur,int step,
        ...
        for(int bay=0;bay<n_bays;bay++){
            ...
            bool use_sweep=RASTER&&maxLb>0&&bayH>0&&bayH<20000;
            const bool HARDREJ=HARDREJ_on();
            // OR-STAMP GRID CACHE (an incremental stamp grid): the (occ,F) pair
    ...
    // wastes floor but keeps them whole.  The instances split the other way on P3,
    ...
    contact_beam(std::vector<int> order, std::vector<double> areas, std::vector<double> workloads,
        ...
        for(int level=0; level<nord; level++){
            ...
            // identical choices, which candidate generation already prevents.  A depth-consistent beam
            ...
            // PER-PARENT QUOTA (children per parent capped at max(2,(B+1)/2)).
    ...
    // Wide-beam method: keep B partial states ("multiverses"); branch the highest-priority
    ...
    // FREE-CAPACITY-INTEGRAL Z1 tail heuristic (_h_z1):
    ...
    // (flat = placed blocks) -- the beam's h_z1 rank term.  Lets a Python contact
    ...
    // MILESTONE 1: fixed-order depth-consistent beam with the
    ...
    // kept per state (objective-selected), cand_per_state.
    std::pair<double,std::vector<int>>
