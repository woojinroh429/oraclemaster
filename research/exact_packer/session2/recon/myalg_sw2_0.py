...
def _contact_beam(prob_info, deadline_s, B=24, K=4, pos_lam=0.1, prefw=0.0, order="edd", mum=1.0,
                  cohort=0.0, shadow=0.0, span=0.0, lex=0, shadoww=0.0,
            ...
            try:
                if _anchor is not None:
                    _ob, _flat = E.contact_beam(order_ids, areas_l, wl, int(B), int(K), int(step),
                                                ...
                                                float(_sc), 1.0, 0.01, float(cohort), float(shadow), float(span), int(lex), float(shadoww))
                else:
                    _ob, _flat = E.contact_beam(order_ids, areas_l, wl, int(B), int(K), int(step),
                                                float(pos_lam), float(prefw), float(mu),
                                                float(w1), float(w2), float(w3_route), float(fut_beta),
    ...
    for step, frac in ((1, 0.6), (2, 1.0)):
        ...
        try:
            r = _contact_beam(prob_info, left, B=(1 if cfg.get("lex") else _beam_width(cfg["Bmul"])),
                              ...
                              w3mul=cfg["w3mul"], cohort=cfg.get("cohort", 0.0), shadow=cfg.get("shadow", 0.0), span=cfg.get("span", 0.0), lex=cfg.get("lex", 0.0), shadoww=cfg.get("shadoww", 0.0), step=step)
        except Exception:
            r = None
        if r:
            s = _recs_to_ops(r, n)
...
#             crane descents.
_AXES = [
    dict(Bmul=1.0, K=4, pos_lam=0.10, order="defer_big", fut_beta=1.0, prefw=0.0, w3mul=1.0, cohort=0.0, dk=3),
    dict(Bmul=1.0, K=4, pos_lam=0.12, order="defer_big", fut_beta=1.0, prefw=0.0, w3mul=3.0, cohort=0.3, shadoww=2.0, dk=3),
    dict(Bmul=0.7, K=5, pos_lam=0.15, order="lst",       fut_beta=0.0, prefw=0.0, w3mul=3.0, cohort=0.3, shadoww=2.0, dk=3),
    dict(Bmul=0.7, K=5, pos_lam=0.05, order="edd",       fut_beta=1.5, prefw=0.0, w3mul=1.0, cohort=0.3, shadoww=2.0, dk=3),
    dict(Bmul=1.4, K=3, pos_lam=0.10, order="big_first", fut_beta=0.5, prefw=0.0, w3mul=6.0, cohort=0.3, shadoww=2.0, dk=3),
    dict(Bmul=0.5, K=6, pos_lam=0.20, order="defer_big", fut_beta=0.0, prefw=0.0, w3mul=1.5, cohort=0.0, dk=3),
]
...
def _regrow(prob_info, sol, budget, cfg, stay, share=1.0, anchor=None, mum=1.0):
    ...
    try:
        r = _contact_beam(prob_info, budget, B=B, K=cfg["K"], pos_lam=cfg["pos_lam"],
                          ...
                          mum=mum, cohort=cfg.get("cohort", 0.0), shadow=cfg.get("shadow", 0.0), span=cfg.get("span", 0.0), lex=cfg.get("lex", 0.0), shadoww=cfg.get("shadoww", 0.0))
    except Exception:
        return None
