...
# 10:07: this queue had set CURRENT=beamhard, the container restarted, the reset put `idle`
...
( cd "$(git rev-parse --show-toplevel)" \
  ...
  && git commit -q -m "queue: CURRENT=beamhard" \
...
run(){ # rep arm prob env
    ...
    ( cd "$(git rev-parse --show-toplevel)" \
      && git add research/exact_packer/session2/recon/results/audit/beamhard.log research/exact_packer/session2/recon/harness/CURRENT \
