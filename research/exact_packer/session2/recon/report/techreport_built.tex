...
One tool was used, and substantially. Anthropic Claude, through its command-line coding interface,
served as an interactive coding and analysis assistant throughout development. It wrote and
refactored the two C++ extensions and the Python scheduler from specifications settled in
discussion, derived the contact bound of Section~\ref{sec:succeeded} together with its validity
conditions, built the measurement harnesses, ran the experiments, diagnosed defects among them the
position-dependent geometric predicate, and drafted this report. Algorithmic direction,
prioritisation and every decision to accept or reject a change remained with the authors. Every
number here came from executing the submitted code. None was generated or estimated by a language
model.
