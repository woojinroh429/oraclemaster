...
The headroom from that build then bought nothing: widening the $40$-outsider candidate cap became
affordable ($331$\,s of a $180$\,s budget before, $179$\,s after) and scored $3$ better against $4$
worse, median $+0.17\%$ --- the evidence for wanting it had come from two instances.
...
an interactive coding and analysis assistant throughout development. It wrote and refactored the
two C++ extensions and the Python scheduler from specifications settled in discussion; derived the
contact bound of Section~\ref{sec:succeeded} and its validity conditions; built the measurement
harnesses; ran the experiments; diagnosed defects, among them the position-dependent geometric
predicate; and drafted this report. Algorithmic direction, prioritisation and every decision to
accept or reject a change remained with the authors, and every number here came from executing the
submitted code --- none was generated or estimated by a language model.
