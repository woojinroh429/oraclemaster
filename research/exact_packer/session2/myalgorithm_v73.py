...
def _vlns_refine(prob_info, assign, bay_unit, deadline, seed=7):
    ...
    try:
        ...
        def cp_place(J, cand, frozen_place, weights, sd, step=8, single_entry=True, tl=0.05):
            ...
            res = CP.pack(blocks_in, float(W), float(H), step, tl, seed=sd, warm=None,
        ...
        def window_repack(a, r, sd, WIN=6, npull=2):
            ...
            placed = cp_place(J, window, frozen, weights, sd)
            if placed is None:
                return None
        ...
        _d0 = min(deadline - 1.0, _t.time() + 0.45 * (deadline - _t.time()))
        assign = _z3_relocate_cp(prob_info, assign, bay_unit, _d0, FREE=10)
...
def _streamlined_lowdensity(prob_info, bay_unit, deadline, rng, worker_id):
    ...
    if HAVE_CRANEPACK and _t.time() < deadline - 3.0:
        try:
            _vseed = 101 + 2 * (worker_id or 0)
            _vdl = min(deadline - 1.0, _t.time() + 0.90 * (deadline - _t.time()))
            av = _vlns_refine(prob_info, {b: dict(v) for b, v in best_a.items()},
                              bay_unit, _vdl, seed=_vseed)
            ov = _score(av)
            if ov < best_obj:
    ...
    # SHAKE-CONTINUATION: the exact assignment converges fast (~30s) to a plateau;
...
def _solve_once_impl(prob_info, timelimit=60, seed=12345,
    ...
    if (os.environ.get("STREAMLINE", "1") == "1" and HAVE_ORTOOLS
        ...
        try:
            _sr = _streamlined_lowdensity(prob_info, bay_unit, deadline, rng, worker_id)
        except Exception:
            _sr = None
        if _sr is not None:
            _sol, _obj = _sr
