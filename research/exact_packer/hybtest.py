"""HYBRID: C++ cranepack builds columns+conflict-graph+heuristic warm-start (fast),
then Gurobi solves the set-packing to PROVEN optimum on the precomputed edges.
Measures model-build vs solve time separately. Exact-verifies the result."""
...
HTL=float(sys.argv[4]) if len(sys.argv)>4 else 0.2   # C++ heuristic budget
GTL=float(sys.argv[5]) if len(sys.argv)>5 else 30    # gurobi budget
...
# ---- C++ build + heuristic ----
t0=time.time()
hbest,hpl,ncol,nedge,build_ms,solve_ms,coldesc,edges,warmcols=CP.pack(
    blocks_in,float(W),float(H),STEP,HTL,seed=12345,warm=warm)
cpp_t=time.time()-t0
coldesc=np.asarray(coldesc); edges=np.asarray(edges); warmcols=list(warmcols)

# ---- Gurobi on precomputed graph ----
tb=time.time()
byb=defaultdict(list)
for ci in range(ncol): byb[int(coldesc[ci,0])].append(ci)
m=gp.Model("hyb"); m.setParam("OutputFlag",0); m.setParam("TimeLimit",GTL); m.setParam("Threads",4); m.setParam("MIPFocus",1)
y=[m.addVar(vtype=GRB.BINARY) for _ in range(ncol)]
pl={b:m.addVar(vtype=GRB.BINARY) for b in range(len(clique))}
for b in range(len(clique)):
    m.addConstr(gp.quicksum(y[ci] for ci in byb[b])==pl[b])
# pairwise conflict constraints from precomputed edges
for a,bb in edges:
    m.addConstr(y[int(a)]+y[int(bb)]<=1)
m.setObjective(gp.quicksum(pl[b] for b in range(len(clique))), GRB.MAXIMIZE)
# warm start from heuristic
for v in y: v.Start=0
for ci in warmcols: y[ci].Start=1
build_t=time.time()-tb
ts=time.time(); m.optimize(); gsolve_t=time.time()-ts
gbest=int(round(m.ObjVal)); gap=m.MIPGap

# selected
selcols=[ci for ci in range(ncol) if y[ci].X>0.5]
sel=[(clique[int(coldesc[ci,0])],int(coldesc[ci,1]),int(coldesc[ci,2]),int(coldesc[ci,3])) for ci in selcols]
bad=0
...
print(f"{NAME} bay{j} clique={len(clique)} step{STEP}:")
print(f"  C++ : heur={hbest} greedy={gc}  cols={ncol} edges={nedge}  [build{build_ms:.0f} heur{solve_ms:.0f} = {cpp_t*1000:.0f}ms]")
print(f"  GRB : optimum={gbest} (gap={gap:.3f})  [model-build{build_t*1000:.0f}ms solve{gsolve_t*1000:.0f}ms]  bad={bad}")
print(f"  >>> TOTAL C++heur+Gurobi = {(cpp_t+build_t+gsolve_t)*1000:.0f}ms  final={gbest} (greedy {gc})")
