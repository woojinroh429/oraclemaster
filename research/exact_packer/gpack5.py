"""gpack4 + greedy warm-start + Gurobi tuning (symmetry, cuts, MIPFocus).
Test whether step-6 clique solves to optimal FAST."""
...
# ---- greedy first (for warm-start + baseline) ----
...
def footprint(bid,o):
    ...
    gs=[g for g in bms if g[0] is not None]
    if not gs: fpcache[key]=(None,0,0); return fpcache[key]
    minx=min(g[1] for g in gs); miny=min(g[2] for g in gs)
    maxx=max(g[1]+g[0].shape[1] for g in gs); maxy=max(g[2]+g[0].shape[0] for g in gs)
...
for bid in clique:
    ...
    for o in range(len(bd["shape"])):
        ...
        ys,xs=np.where(U)
        # ensure greedy position is a column even if off-grid
        gx=set(range(xlo,xhi+1,STEP)); gy=set(range(ylo,yhi+1,STEP))
        if bid in gplace and gplace[bid][0]==o:
            gx.add(gplace[bid][1]); gy.add(gplace[bid][2])
        for x in sorted(gx):
            for y in sorted(gy):
                if not (xlo<=x<=xhi and ylo<=y<=yhi): continue
                cols.append(dict(bid=bid,x=x,y=y,o=o,en=en,ex=ex,
                                 scx=xs+minx+x*Rr, scy=ys+miny+y*Rr))
tcol=time.time()-t0
cell2cols=defaultdict(list)
for ci,c in enumerate(cols):
    for sx,sy in zip(c["scx"],c["scy"]): cell2cols[(sx,sy)].append(ci)

m=gp.Model("p"); m.setParam("OutputFlag",0); m.setParam("TimeLimit",30); m.setParam("Threads",4)
m.setParam("MIPFocus",2); m.setParam("Symmetry",2); m.setParam("Cuts",2)
...
for cell,cl in cell2cols.items():
    if len(cl)>1: m.addConstr(gp.quicksum(y[i] for i in cl)<=1); ncon+=1
m.setObjective(gp.quicksum(pl.values()), GRB.MAXIMIZE)
# warm start: greedy columns
gcolidx={}
for ci,c in enumerate(cols):
...
for b in clique: pl[b].Start=1 if b in gcolidx else 0
ts=time.time(); m.optimize(); tsolve=time.time()-ts
sel=[i for i in range(len(cols)) if y[i].X>0.5]; got=int(round(m.ObjVal))
bad=0
...
print(f"{NAME} bay{j} clique={len(clique)} step{STEP}: GUROBI={got} (gap={m.MIPGap:.3f}) vs GREEDY={gc} "
      f"| cols={len(cols)} cliques={ncon} bad={bad} [col{tcol:.1f} solve{tsolve:.1f}s] warmstart={len(gcolidx)}",flush=True)
