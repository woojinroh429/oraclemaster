"""Fast clique packer: numba conflict + Gurobi set-packing + exact-verify.
Compare Gurobi max on-time placement vs greedy on bay1's peak clique."""
...
print(f"{NAME} bay{j} {W}x{H} peak t={best_t}: clique={len(clique)} blocks", flush=True)

# build columns (entry=release)
t0=time.time(); cols=[]
for bid in clique:
    ...
    for o in range(len(bd["shape"])):
        ...
        for x in range(xlo,xhi+1,STEP):
            for y in range(ylo,yhi+1,STEP):
                ...
                bbabs=(allpts[:,0].min(),allpts[:,1].min(),allpts[:,0].max(),allpts[:,1].max())
                cols.append(dict(bid=bid,x=x,y=y,o=o,en=en,ex=ex,arr=Aa,poly=Ap,bb=bbabs))
tcol=time.time()-t0
# conflicts (numba)
t0=time.time(); E=[]
for a in range(len(cols)):
    ...
    for b in range(a+1,len(cols)):
        ...
        if FC.conflict(ca["arr"],ca["poly"],ca["en"],ca["ex"],cb["arr"],cb["poly"],cb["en"],cb["ex"]):
            E.append((a,b))
tconf=time.time()-t0

def build_solve(E, tl):
    ...
    return int(round(m.ObjVal)), sel, m.MIPGap

# exact-verify + cut loop (guard against rare numba/shapely conflict misses)
tl=90; extra=0
for _round in range(6):
    ts=time.time(); got,sel,gap=build_solve(E,tl); tsolve=time.time()-ts
    # verify selected columns pairwise with grader_conflict (exact)
    bad=[]
    for ii in range(len(sel)):
        for jj in range(ii+1,len(sel)):
            ...
            if CG.grader_conflict(Bay(width=W,height=H,id=j),A,Bk,ca["en"],ca["ex"],cb["en"],cb["ex"]):
                bad.append((sel[ii],sel[jj]))
    if not bad:
        break
    E.extend(bad); extra+=len(bad)
print(f"{NAME} bay{j} clique={len(clique)}: GUROBI={got} (gap={gap:.2f}, +{extra} exact-cuts) "
      f"| cols={len(cols)} conf={len(E)} [col{tcol:.1f}s conf{tconf:.1f}s solve{tsolve:.1f}s]", flush=True)

# greedy on same clique (entry=release)
...
print(f"  --> GUROBI={got} vs GREEDY={gc}  (delta +{got-gc})", flush=True)
