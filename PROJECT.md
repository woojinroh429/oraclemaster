...
## "부하문제인가?" — 불균형 아니라 포화(saturation), 재분배 불가
사용자 가설: 지각이 부하(load) 문제인가. 대기 시점 bay별 점유율 측정(load_diag.py):
- prob_28: 가장 빈 bay 평균 78%, 가장 찬 bay 100%, 불균형 21%. 최악 t=16: [100%,92%,88%].
- prob_30: 가장 빈 bay 87%, 찬 bay 98%, 불균형 11%. 최악 t=18: [110%,106%].
판정: **불균형(imbalance) 아니라 포화(saturation).** 블록 대기 시 모든 bay가 78~110%로 참 →
옮겨갈 빈 bay 없음. 확정: (1) Z2(부하불균형) 목적은 레버 아님(가중치 0~5%+실제 불균형 11~21%로
작음 → _exact_reassign 재배정이 P4에서 무효였던 이유=옮길 곳 없음). (2) 지각 근본=부하가 총
크레인용량 초과(misallocation 아니라 over-demand). 릴리스 고정→수요 못 줄임, 전 bay 포화→재분배
불가. 유일한 길="같은 공간에 더 많이"=더 조밀한 크레인패킹인데 idea #1로 최적 증명(못 뚫음).
부하는 지각의 정확한 원인이나 분배문제(고침가능)가 아니라 총량문제(고침불가). 스크립트: load_diag.py.

## 저밀도에 고밀도 패킹+LNS도 돌리기 (HDSPLIT/FORCEHD) — validated-NEGATIVE
사용자 아이디어: 저밀도라도 지각을 잘 잡게, 저밀도 게이트가 "전부 아니면 전무"라 저밀도면 4워커
전부 재배정 경로 → 고밀도 패킹+LNS를 아무도 안 함. 이걸 (FORCEHD=전워커 HD / HDSPLIT=짝수워커
HD+홀수워커 재배정 best-of) 시험.
결과(prob_20/22/24/29): **전부 BASE가 최선.**
- prob_20: BASE 86517, HDSPLIT 163339(2배악화), FORCEHD 171983(2배)
- prob_22: 794496 vs 819935 vs 1009408
- prob_24: 578452 vs 590767 vs 578452
- prob_29: BASE 394560
원인: (1) 저밀도는 Z1=0이라 ALNS 패킹이 잡을 지각이 없음, (2) HD가 재배정 tail(_exact_reassign,
...
## 심층 수학 분석 — 지각은 용량이 아니라 크레인 기하가 강제함 (엄밀 증명)
사용자 요청: 문제·알고리즘의 수학 원리 심층. 결과:
- **정식화**: MIP, 비볼록 기하(층별 non-overlap + 크레인 하강 disjunctive) → NP-hard. 다항 스케줄링/flow
  bound으로 환원 불가.
- **하한(엄밀)**: 크레인-완화 cumulative LP(CP-SAT, 베이 pool, 면적수요) → **Z1=0** (prob_34/28).
  즉 용량만 문제면 지각 0 가능. opt(완화)≤opt(실제)≤v61이므로 유효 하한. prob_38은 [18,423].
- **크레인 포화 증명서**: v61 해 분석 — 블록이 대기(지각)하는 순간 베이는 **면적 55~58%만 참**
  (prob_34 median 55%, prob_28 58%). 즉 공간 ~42% 비었는데 크레인 하강로가 막혀 진입 불가.
  "80% 차도 안 들어감"의 실측 = 55% 차도 크레인이 막음.
- **결론(엄밀 연쇄)**: 용량 하한=0인데 지각>0, 대기시 55% 이용률 → 지각은 **크레인 기하가 강제**
  (용량 아님). 3DTCS(공간-시간 접촉최대=조각화최소)가 **크레인 도달가능 이용률**을 올려 정확히
